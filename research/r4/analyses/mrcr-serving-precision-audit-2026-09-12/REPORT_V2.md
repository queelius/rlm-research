...
some movements; that is quantization error, not evidence of a stronger update. More directly, the
effective B@A update direction remains highly aligned after the storage cast: cosine
0.906 for
fixed-baseline RL1 and 0.913
for fresh RLOO1. Their unscaled effective-matrix delta norms change from
0.01641
to 0.01812,
and 0.01813
to 0.01986,
respectively. This argues against complete serving-time erasure while retaining a material
...
Serving precision is a plausible noise contributor for LR=1e-5 one-step updates, not a complete or
leading explanation for weak RL: effective update directions retain >0.90 cosine and neither update
vanishes. A targeted comparison is lower priority than task/interface/data experiments, but becomes
warranted if a small already-frozen decoding panel can directly serve the *same fixed checkpoint*
with `lora_dtype=bfloat16` versus `lora_dtype=float32` without new compatibility work. This would be
a serving-precision mechanism screen, not another training arm. It must preserve all seeds/prompts
and report runtime/memory compatibility; FP32 serving is currently only a counterfactual and has not
been run.
