...
status: prospective_frozen_before_outcomes_failure_diagnostic_amendment_v3
...
There are two authorization modes. `clean` is unchanged: it requires exit zero, no timeout, clean
owner completion, no owner error, and owner-recorded release. `failure_diagnostic` accepts a nonzero
exit, timeout, owner error, or incomplete owner only after the same exact terminal EXIT/OWNER hashes
and authenticated release. Release must be recorded by the owner or by a separately hash-relayed MAIN
release receipt that says released and reports no active GPU processes. This mode preserves all 64
planned cells, marks missing/unauthenticated cells `NULL`, and may report failure location,
availability, and incurred physical cost. It cannot authorize a scientific-success claim. The
pre-amendment reader and method bytes are retained under `history/v2/`.
