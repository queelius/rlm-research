"""One56-call yes/no owner with fresh reusable full controls and fixed750-second cap."""
...
from yesno_metrics import secondary, summarize


def execute(cap):
    ready = study.verify()
    if cap != study.CAP or study.ATTEMPT.exists():
        raise ValueError("unused attempt001 and exact750-second cap required")
    ...
    def stop(sig, _frame):
        raise TimeoutError("yes/no comparison interrupted by signal " + str(sig))
    ...
    try:
        ...
        for dataset in summary["datasets"].values():
            ...
            screen["passes"] = screen["passes"] and qualified
        summary["secondary_verbalizer_diagnostic"] = secondary(
            summary,
            study.read(study.OLD_RESULT),
            study.read(study.ROOT / "MANIFEST.json")["secondary_old_complete_blocks"],
        )
