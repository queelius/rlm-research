"""MAIN-launched one-service owner with an inclusive 1,800-second envelope."""
...
def credential_preflight():
    ...
    spec = importlib.util.spec_from_file_location("role_tool_credential_preflight", path)
...
def load_suite():
    ...
    spec = importlib.util.spec_from_file_location("role_tool_owned_suite", SUITE)
...
def base_preflight(directory, binding):
    ...
    service.validate_descriptor(endpoint)
    ...
    service.validate_models(models.json())
...
def binding():
    return {"schema": "released-base-single-model-binding-v1", "model": "qwen3",
            "checkpoint": s.MODEL, "weights_sha256": s.sha(s.ROOT / "WEIGHTS.json"),
            "adapter": None}
...
def collector_argv(stage, output, deadline):
    ...
    if Path(argv[6]).resolve() != driver.AUTHORIZED_OUTPUT.resolve():
        raise ValueError("owner-to-collector output namespace differs")
    return argv
...
def execute(output):
    ...
    previous = {sig: signal.signal(sig, expired) for sig in
    ...
    try:
        ...
        collection_deadline = min(work_deadline, time.time() + 1500)
        argv = collector_argv(stage, output, collection_deadline)
        suite.command(stage, "role-tool-collect", argv, 1680, work_deadline)
        terminal = s.read(output / "rollout/STATUS.json")
        if terminal["planned"] != 96 or terminal["recorded"] != 96:
            raise ValueError("collector did not retain all planned records")
    ...
    result = {"complete": error is None and release_error is None and released,
              ...
              "collector_status": terminal, "planned": 96,
              "elapsed_seconds": time.time() - started, "outer_seconds": 1800,
              "no_retry": True, "main_owns_gpu_and_lock": True}
