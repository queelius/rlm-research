"""64 fresh native roots; independently crossed program/observation visibility."""
import argparse
import asyncio
import json
import os
...
async def collect(args):
    ...
    args.output.mkdir(parents=True,exist_ok=False);started=time.time();deadline=min(args.deadline,started+2400)
    ...
    s.write(args.output/'TERMINAL.json',dict(planned=64,recorded=len(final),available=sum(r['available'] for r in final),correct=sum(r['reward']==1 for r in final),
        ...
        interpretation='all-fresh program/observation visibility factorial; not native continuation or fresh-context confirmation'))
