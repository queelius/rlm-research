...
def summarize(rows):
    ...
    return dict(arms={arm:group([r for r in rows if r['coordinate']['arm']==arm]) for arm in p.ARMS},costs=dict(physical_requests=len(costs),requests_with_usage=len(known),unknown_usage=len(costs)-len(known),prompt_tokens=sum(r['usage_observed']['prompt_tokens'] for r in known),completion_tokens=sum(r['usage_observed']['completion_tokens'] for r in known),provider_billing='unknown/not measured'),cluster_unit='8 paired MNLI contexts; labels within a call are not independent clusters')
