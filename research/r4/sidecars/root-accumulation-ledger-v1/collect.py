"""Sixteen immutable free-root slots, paired dispatch and settled cancellation writes."""
import argparse
import asyncio
import os
import time
...
async def collect(args):
    ...
    started=time.time();deadline=min(args.deadline,started+1200);rows=[];order=[]
    async def one(row):
        ...
        try:
            ...
            task=adapter.task(context,tasks[row['task_name']]['prompt'],gold[context['id']],row['task_name'],row)
    ...
    s.write(args.output/'STATUS.json',{'recorded':len(rows),'planned':16,'not_started':missing,'stop_reason':stop,
