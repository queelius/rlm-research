...
def test_actual_owner_to_qualified_service_entry_before_intercepted_model_launch(tmp_path):
    ...
    with s.aliases({'study':s,'protocol':p}):owner=s.load('warm_reference_lifecycle_test_owner',s.ROOT/'owner.py',s.sha(s.ROOT/'owner.py'))
    ...
    def instrumented_spec(name,path,*args,**kwargs):
        ...
        return spec
    output=tmp_path/'attempt';arm=p.phase_order()[0];stage=output/arm/'service'
    ...
    assert not result['complete'] and result['errors'][0]['type']=='InterceptedLaunch'
    ...
    assert binding==s.binding(arm) and endpoint['adapter']['model_sha256']==s.ROOTS[arm]['adapter_sha256']
