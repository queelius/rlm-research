"""Bounded owner for the fixed checkpoint32 grouped rollout screen."""
...
def preflight(service: Path, binding: dict) -> None:
    ...
    study.write_x(service.parent / "PREFLIGHT.json", {
...
def execute(output: Path, outer_seconds: int) -> dict:
    if output.resolve() != OUTPUT.resolve() or output.exists():
        raise ValueError("exact unused fixed output required")
    ...
    study.write_x(output / "OWNER_RUN.json", {
        "phase": "train", "arm": "checkpoint32", "started_epoch": started,
        "owner_seconds": outer_seconds, "science_seconds": study.SCIENCE_SECONDS,
        "planned": 32, "ready_identity": ready["identity"],
        "checkpoint_receipt_sha256": study.sha(checkpoint.RECEIPT),
        "terminal_strip_condition": "terminal-strip-disabled", "optimizer_steps": 0,
    })
    errors, suite, released = [], None, False

    def stop(*_):
        raise TimeoutError("grouped rollout owner signal")
    ...
    try:
        ...
        argv = [str(study.NATIVE), str(study.ROOT / "collect.py"), "--phase", "train",
                "--arm", "checkpoint32", "--endpoint", str(endpoint),
                "--output", str(output / "science"), "--deadline", str(science_deadline)]
        suite.command(service, "mrcr-sft32-onpolicy-screen", argv,
                      min(study.SCIENCE_SECONDS, max(1, science_deadline - time.time())), science_deadline)
        ...
        complete = bool(not errors and released and result and result.get("complete") is True
        ...
        terminal = {"complete": complete, "released": released, "errors": errors or None,
                    ...
                    "elapsed_seconds": time.time() - started, "optimizer_steps": 0, "no_retry": True}
