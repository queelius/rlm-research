...
def run(output):
    ...
    try:
        ...
        deadline=min(work,time.time()+1500)
        ...
        suite.command(stage,'collect',argv,1530,work)
    ...
    terminal={'complete':error is None,'error':error,'elapsed_seconds':time.time()-started,
              'owned_deadline_epoch':owned,'planned':32}
...
if __name__=='__main__':
    ...
    if a.command=='verify':s.verify();print('CPU verified; 32 planned; GPU/model calls0')
    else:
