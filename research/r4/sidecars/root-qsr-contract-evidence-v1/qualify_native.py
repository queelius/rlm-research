...
async def qualify(output):
    ...
    rows=[{**row,'id':'CONTRACT_CPU_'+str(i)} for i,row in enumerate(value['PLAN.json'][:4])]
    maps=json.dumps({r['id']:'entity' for r in context['records']},separators=(',',':'))
    tasks={r['id']:n.task(context,value['QUERIES.json'][r['task_name']]['question'],r,maps if r['evidence']=='map' else None) for r in rows}
    expected={r['id']:n.expected(tasks[r['id']],r) for r in rows};binding=s.binding();root=binding['role_map']['root'];child=binding['fixed_child']
    ...
    endpoint=dict(url=f'http://127.0.0.1:{site._server.sockets[0].getsockname()[1]}/v1',model=root,renderer_model=str(s.qnative().stack().prior.BASE),api_key_env='CONTRACT_CPU_PROVIDER_KEY')
    before={k:os.environ.get(k) for k in ('STRICT_RLM_CALIBRATION_API_KEY','CONTRACT_CPU_PROVIDER_KEY')}
        ...
        with n.installed(interface,binding,output,rows,{context['id']:context},expected):
            ...
            async with env.serving():
                for row in rows:
                    ...
                    assert trace['info']['contract_evidence_setup']['file_sha256']['records.json']==__import__('hashlib').sha256(json.dumps(context['records'],ensure_ascii=False).encode()).hexdigest()
                    child_record=next(v for v in roles.values() if v['depth']>0)
                    assert child_record['native_request']['messages'][0]['content']!=p.COMPACT
        assert len(calls)==12
        records=list(c.audits(output).values());assert c.cost(records)['returned_native_completions']==12
        systems={r['role']:next(v['native_request']['messages'][0] for v in records if v['coordinate']['id']==r['id'] and v.get('first_prefix_verified')) for r in rows}
        assert systems['compact']['content']==p.COMPACT and systems['native']!=systems['compact']
        s.write(output/'RESULT.json',dict(passed=True,actual_native_episodes=4,native_calls=12,child_calls=4,root_calls=8,all_first_prefixes_verified=True,child_typed_grammar_unchanged=True,child_system_unchanged=True,exact_public_files=True,authored_code=code,gpu_calls=0,model_service_calls=0,elapsed_seconds=time.time()-started,source_sha256={str(s.ROOT/name):s.sha(s.ROOT/name) for name in ('study.py','protocol.py','native.py','collect.py','qualify_native.py')}))
        print(dict(passed=True,native_calls=12,elapsed_seconds=time.time()-started))
