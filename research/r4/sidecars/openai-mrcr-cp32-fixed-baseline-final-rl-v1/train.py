"""One fixed-baseline conditional-final REINFORCE step; no sampling or mixed gate."""
import argparse
import math
import os
...
def qualify(model, rows):
    ...
    diagnostics.update(objective='fixed_baseline_conditional_final_REINFORCE', baseline=.5,
                       denominator=32, exact_sequence_IS_claim=False, full_RLM_IS_claim=False,
...
def accumulate(model, rows, baseline, weights, output):
    ...
    for i, row in enumerate(rows):
        assert row['advantage'] == row['reward'] - .5 and len(row['root_turns']) == 1
...
def run(output, seconds):
    ...
    try:
        ...
        state = dict(schema='cp32-fixed-baseline-final-RL-state-v1', status='UPDATED',
            ...
            weight_decay=0., betas=[.9,.999], eps=1e-8, gradient_clip=1., baseline=.5, denominator=32,
            groups=8, trajectories=32, exact_reward_positive=28, exact_reward_negative=4,
            actual_final_tokens=10420, zero_loss_other_root_tokens=6716, child_loss_tokens=0,
            policy_objective='conditional final sampled tokens, fixed action-independent baseline',
            ...
            replay_actions=32, mixed_group_gate=False, parent_adapter_sha256=study.ADAPTER_SHA,
        ...
        binding.update(schema='cp32-fixed-baseline-final-RL-binding-v1',
                       ...
                       selection_semantics='Adaptively proposed objective after cp32 rollout diagnosis; fixed one-step endpoint.',
