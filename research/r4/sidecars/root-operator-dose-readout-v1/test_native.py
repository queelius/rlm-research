...
def test_authored_native_file_child_reduction_final(tmp_path,monkeypatch):
    path=Path(__file__).with_name('dr_collect.py');assert path.exists(),'qualified free collector missing'
    ...
    async def fixture():
        row=next(r for r in s.read(s.ROOT/'inputs/FREE_PLAN.json') if r['stratum']=='root_new' and r['operator']=='count' and r['heldout_cell'])
        ...
        expected=sum(r['user'] in row['users'] for r in context['records'])
        ...
        code2=f'answer=sum(labels[r["id"]]=={row["target"]!r} for r in records if r["user"] in {row["users"]!r})\nprint(answer)'
