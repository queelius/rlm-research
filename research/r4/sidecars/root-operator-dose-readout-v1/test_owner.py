...
def test_actual_owner_service_entry_and_two_lora_config(tmp_path):
    assert Path(__file__).with_name('dr_owner.py').exists(),'readout owner missing'
    import dr_study as s
    import dr_owner as o
    ...
    def instrument(name,path,*a,**kw):
        ...
        return spec
    output=tmp_path/'attempt';stage=output/'service-sft6'
    ...
    assert endpoint['adapter']['model_sha256']==s.dose.START_SHA and endpoint['role_binding_sha256']==s.sha(stage/'BINDING.json')

def test_exact_collector_and_probe_argv_namespaces(tmp_path,monkeypatch):
    assert Path(__file__).with_name('dr_owner.py').exists(),'readout owner missing'
    import dr_study as s
    import dr_owner as o
    ...
    args=p.parse_args(o.probe_argv(stage,tmp_path/'sft6/probes',124.)[2:]);assert args.deadline==124. and args.binding==stage/'BINDING.json'
