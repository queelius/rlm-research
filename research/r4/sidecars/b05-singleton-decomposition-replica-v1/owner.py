"""Unchanged qualified service lifecycle; new328 physical-call admission only."""
import argparse
import json
import types
import collect
...
def implementation():
    with study.aliases({'runner_study':study.source},study.SOURCE):v3=study.load('singleton_replica_owner_v3',study.SOURCE/'runner_owner_v3.py')
    _,text=v3.owner_sources()
    for field in ('planned_coordinates','planned_physical_cap'):
        before=f'"{field}": 64';assert text.count(before)==1;text=text.replace(before,f'"{field}": 328')
    module=types.ModuleType('singleton_replica_native_owner');module.__file__=__file__;module.lifecycle=v3.lifecycle_module()
