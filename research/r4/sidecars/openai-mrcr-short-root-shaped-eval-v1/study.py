"""Frozen one-rollout heldout schedule for the short-MRCR shaped-root update."""
...
TRAINING = SIDE / "openai-mrcr-short-root-shaped-rl-v1"
TRAIN_OUTPUT = TRAINING / "outputs/attempt-001"
RENDER_SOURCE = SIDE / "openai-mrcr-procedural-sft-warmstart-v1"
EVAL_SOURCE = SIDE / "openai-mrcr-procedural-sft-eval-v1"
BASE = Path(
...
BASE_ALIAS = "Qwen3-4B-Instruct-2507-shaped-root-eval-zero"
UPDATED_ALIAS = "Qwen3-4B-Instruct-2507-mrcr-shaped-root-step1"
OWNER_SECONDS = 650
SCIENCE_SECONDS = 500
...
def source():
    return load("shaped_root_eval_short_source", SOURCE / "study.py")
...
def records(phase: str):
    if phase != "held":
        raise ValueError("only the frozen held phase is available")
    return model_inputs()["heldout"]
...
def prepare_inputs() -> dict:
    ...
    teacher = load("shaped_root_eval_teacher_renderer", RENDER_SOURCE / "teacher.py")
    ...
    for index, coordinate in enumerate(schedule(phase)):
        ...
        tasks.append(
            old.MRCRData(
                ...
                arm="shaped_root_held",
            ).model_dump(mode="json", exclude_none=True)
        )
    source_gold = read(DATA / "host/HOST_GOLD.json")
    gold = {row_id: source_gold["heldout"][row_id] for row_id in selected}
    prefixes = {}
...
def environment(phase: str):
    ...
    async def setup(self, trace, runtime):
        ...
        if path is None:
            raise ValueError("task references context outside frozen held split")
