"""Run one arm's authenticated checkpoint-0004 on frozen unseen256."""
...
def build(arm):
 study.select(arm);source=study.C32/"owner.py";spec=importlib.util.spec_from_file_location("arm_unseen_collector_owner",source);collector=importlib.util.module_from_spec(spec);previous=sys.modules.get("unseen_panel_study");sys.modules["unseen_panel_study"]=study
