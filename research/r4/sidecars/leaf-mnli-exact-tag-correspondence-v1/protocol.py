...
def plan():
    ...
    for block in blocks:
        ...
        for position,arm in enumerate(order):
            row=dict(context_index=ci,repeat=repeat,seed=981581101+block,arm=arm,decoder='exact_requested_tag',
                     size=48,block=block,pair_position=position,dispatch_order=len(result))
            row['id']=s.digest(['mnli-exact-requested-tag32',row]);result.append(row)
    return result
