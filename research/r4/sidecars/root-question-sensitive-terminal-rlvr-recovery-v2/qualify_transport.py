...
async def qualify():
    ...
    output = study.ROOT / "qualification-transport24-attempt-003"
    output.mkdir(exist_ok=False)
    started = time.time()
    public, host = study.data()
    contexts = {row["id"]: row for row in public}
    rows = study.candidate_plan(1)
    ...
    try:
        ...
        if stop is not None or len(calls) != 24 or len(raw) != 24 or observed != expected:
            raise ValueError("actual 24-slot transport did not complete frozen repaired tasks")
        result = {"status": "PASS", "provider_calls": 24, "completed_slots": 24,
                  "workers": 4, "actual_model_calls": 0,
