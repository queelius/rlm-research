"""Fresh RL cursor over exact QS6 with qualified group-relative numerics."""
...
with study.aliases({"qsr_study": study}):
    qualified = study.load("terminal_qualified_qsr_common", SOURCE, PIN)
c = qualified.c
...
def starting_decision():
    ...
    if value["policy"] != policy or value["kind"] != "exact_qs6" or not value["fresh_rl_adam"]:
        raise ValueError("starting decision is not exact QS6 with fresh RL Adam")
...
def authenticate_policy(policy):
    if policy["step"] == 0:
        if policy != study.fixed_start():
            raise ValueError("not exact QS6/fresh Adam0")
    else:
        _authenticate(policy)


def check_generation(generation, policy, optimizer_step):
    if study.digest({k: v for k, v in generation.items() if k != "generation_id"}) != generation["generation_id"]:
    ...
    if not 1 <= generation["round"] <= 8 or generation["round"] != policy["step"] + 1:
        raise ValueError("RL cursor outside fixed8")
...
def generation(window, policy):
    ...
    value["generation_id"] = study.digest({k: v for k, v in value.items() if k != "generation_id"})
