"""MAIN-only bounded owner: two trainings, then two dual-LoRA readout services."""
...
def binding(output, aliases):
    ...
    paths = {
        "c32": s.START,
        "full6_sft24": training / "full6/checkpoint-0024",
        "abo_sft24": training / "abo/checkpoint-0024",
    }
    ...
    s.write(
        decision,
        {
            "schema": "trec-child-interface-fixed-binding-v1",
            "models": models,
    ...
    result["selection_semantics"] = (
        "fixed true completed update24 for both continuation adapters; "
...
def execute(output):
    ...
    s.write(
        output / "OWNER_RUN.json",
        {
            ...
            "training_setup_cap_seconds": 2700,
            "readout_cap_seconds": 6300,
            "cleanup_recovery_seconds": 1800,
    ...
    try:
        run_training(output / "training", min(work, time.time() + 2700))
        stages = [
            ("c32-full6", ("c32", "full6_sft24"), ("c32", "full6_sft24"), 3150),
            ("full6-abo", ("full6_sft24", "abo_sft24"), ("abo_sft24",), 3150),
        ]
        ...
        combined = []
        for name, policies in (("c32-full6", {"c32", "full6_sft24"}), ("full6-abo", {"abo_sft24"})):
        ...
        terminal = {
            "identity": ready["identity"],
            "complete": not errors and len(combined) == 480,
            "released": not active,
