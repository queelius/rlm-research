...
def main():
    ...
    if [len(train[arm]) for arm in recipe["arms"]] != [96, 96]:
        raise ValueError("training arms")
    if [sum(row["target_tokens"] for row in train[arm]) for arm in recipe["arms"]] != [
        23999,
        22430,
    ]:
        raise ValueError("training token closure")
    if len(plan) != 480 or len(bodies) != 480 or {row["id"] for row in plan} != set(bodies):
    ...
    ready = {
        "schema": "trec-child-interface-sft-followup-ready-v1",
        "created_epoch": time.time(),
        "question": (
            "Does direct ABO continuation teach query-pair binding beyond "
            "output-contract adaptation?"
        ),
