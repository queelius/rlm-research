...
class OwnerTests(unittest.TestCase):
    def test_actual_owner_start_failure_keeps32_slots_and_cleanup_budget(self):
        import os
        import tempfile
        ...
        with tempfile.TemporaryDirectory(prefix='interface-owner-failure-') as directory:
            ...
            self.assertEqual(run['work_deadline']-run['started_epoch'],1380)
            self.assertEqual(run['owned_deadline']-run['started_epoch'],1470)
            self.assertEqual(run['outer_seconds'],1500)
            self.assertEqual(len(original_read(output/'PLANNED_NULL_ENDPOINTS.json')),32)
    ...
    def test_actual_collector_cli_is_bound_to_new_attempt(self):
        ...
        with self.assertRaises(ValueError):
            owner.validate_argv([x.replace('root-fresh-join-externalization-order-v1', 'root-partition-final-interface-v1') for x in argv])



    def test_actual_owner_through_service_wrapper_intercepted_launch(self):
        import importlib.machinery as machinery
        import importlib.util as util
        import os,subprocess,sys,tempfile
        from types import SimpleNamespace
        from unittest.mock import patch
        import owner,study as s,protocol as p,service_wrapper
        observed={};worlds=p.worlds()
        class Loader:
            def create_module(self,spec):return None
            def exec_module(self,module):
                module.PRIME_ENV=Path('/fixture/prime')
        ...
        with tempfile.TemporaryDirectory(prefix='fresh-owner-service-') as tmp:
            output=Path(tmp)/'attempt-001'
            def start(stage,binding,deadline):
            ...
            def command(stage,name,argv,timeout,deadline):
                parsed=owner.validate_argv(argv);observed['collector']=argv
                s.write(parsed['output']/'STATUS.json',dict(planned=32,recorded=32))
            runner=SimpleNamespace(start_service=start,command=command,release_service=lambda stage:None)
            original_read=s.read;original_sha=s.sha
            def read(path):return p.plan(worlds) if Path(path)==s.ROOT/'PLAN.json' else original_read(path)
            with patch.object(util,'spec_from_file_location',spec),patch.object(subprocess,'Popen',spawn),patch.dict(os.environ,{'CUDA_VISIBLE_DEVICES':'fixture-only','STRICT_RLM_CALIBRATION_API_KEY':'fixture-only'}),patch.object(s,'ATTEMPT',output),patch.object(s,'verify',lambda:{'identity':'cpu'}),patch.object(s,'read',read),patch.object(s,'sha',lambda path:'cpu' if Path(path)==s.READY_PATH else original_sha(path)),patch.object(owner,'credential',lambda:{}),patch.object(owner,'suite',lambda:runner):
                result=owner.execute(output)
            self.assertTrue(result['complete']);self.assertTrue(result['released'])
