"""Evaluate only the authenticated qualified fresh48 child checkpoint."""
...
def build_collector():
    ...
    spec=importlib.util.spec_from_file_location("qualified_one_update_unseen_collector",SOURCE)
    ...
    if collector.study is not study: raise ValueError("collector did not bind one-update study facade")
    return collector
...
def summarize(calls,gold,expected=True):
    ...
    result.update(schema="qualified-fresh48-one-update-unseen-result-v1",
        policy="authenticated qualified fresh48 checkpoint-0001",primary_step=1,
        training_ready_identity=eligibility["training_ready_identity"],
        checkpoint_state_sha256=eligibility["checkpoint_state_sha256"],
        step_commit_sha256=eligibility["step_commit_sha256"],
        qualification_sha256=eligibility["qualification_sha256"],
        eligibility_sha256=study.sha(study.ATTEMPT/"ELIGIBILITY.json"),
        source_c32_ready_identity=study.read(study.C32/"READY.json")["identity"],
