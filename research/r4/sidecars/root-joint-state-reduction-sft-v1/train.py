"""Forward-only4 gate or four complete16-trajectory updates; qualified native LoRA checkpointing."""
import argparse
...
def gate(model,episodes,deadline):
    import torch
    if len(episodes)!=4:raise ValueError('fixed first4 gate')
    model.eval();rows=[];started=time.time()
    with torch.no_grad():
        for episode in episodes:
            result={'episode_id':episode['episode_id'],'roles':[]}
            for turn,_ in l.weighted_turns(episode,'joint'):
                ...
                if turn['kind']=='corrective':
                    spans=turn['span_indices'];flat=[i for values in spans.values() for i in values]
                    if sorted(flat)!=list(range(len(ce))) or not spans.get('mechanism') or spans.get('payload'):raise ValueError('complete native mechanism/literal masks and no payload required')
                    for key,indices in spans.items():row[key+'_nll']=float(ce[indices].mean()) if indices else None
                    result['corrective']=row
                result['roles'].append(row);del ce
            rows.append(result)
    decision=l.objective_gate([r['corrective'] for r in rows])
    projection={arm:sum(t['forward_seconds'] for r in rows for t in r['roles'] if arm=='joint' or t['kind'] in ('corrective','terminal'))/4*16*4*3 for arm in s.ARMS}
    cost_pass=sum(projection.values())<=1200 and max(projection.values())<=600
    return dict(complete=True,pass_gate=decision['pass'] and cost_pass,objective=decision,cost_pass=cost_pass,projected_training_seconds_per_arm=projection,
        projection_method='fixed balanced4 measured selected-role forwards /4 x16trajectories x4updates x3; load/checkpoint overhead separately charged, proxy not guarantee',
        rows=rows,elapsed_seconds=time.time()-started,gradients_performed=0,optimizer_steps=0,
        mechanism_definition='actual accumulation-variable/user-scope/reduction/print code; all Python string literals excluded; no map payload; no gradient share claim')
...
def train(model,params,helper,episodes,arm,output,deadline,identity):
    ...
    for index in range(4):
        order=list(range(16));random.Random(SEED+index).shuffle(order)
        ...
        path=helper.old.save_checkpoint(model,optimizer,output,dict(identity=identity,arm=arm,corpus_sha256=corpus,epoch=index+1,cursor=0,step=index+1,metric=metric,previous_state_sha256=previous,starting_adapter_sha256=s.START_SHA,optimizer_origin='fresh at0; complete16-trajectory current-action passes',elapsed_training_seconds=time.time()-started))
    ...
    return dict(identity=identity,arm=arm,complete=True,selected=selected,optimizer_steps=4,example_exposures=64,root_turn_exposures=sum(x['metric']['root_turns'] for x in states),target_token_exposures=sum(x['metric']['target_tokens'] for x in states),terminal_weight=.05,starting_adapter_sha256=s.START_SHA,corpus_sha256=corpus,fresh_optimizer=True,child_loaded=False,child_updated=False,files_sha256=states[-1]['files_sha256'],elapsed_training_seconds=time.time()-started)

def run(args):
    started=time.time();ready=s.verify();episodes=s.corpus(4 if args.mode=='gate' else None)
    args.output.mkdir(parents=True,exist_ok=False)
    s.write(args.output/'RUN.json',dict(identity=ready['identity'],mode=args.mode,arm=args.arm,seed=SEED,learning_rate=1e-4,terminal_weight=.05,starting_adapter_sha256=s.START_SHA,deadline_epoch=args.deadline,started_epoch=started))
    try:
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--mode',choices=('gate','train'),required=True);ap.add_argument('--arm',choices=s.ARMS,default='joint');ap.add_argument('--output',type=Path,required=True);ap.add_argument('--deadline',type=float,required=True);run(ap.parse_args())
