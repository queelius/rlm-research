...
def binding(arm,chosen):
    ...
    alias='strict-rlm-qwen3-4b-joint-state-'+arm.replace('_','-')+'-v1'
    ...
    value['selection_semantics']='unchanged low66c' if arm=='unchanged' else 'fixed4 role-balanced live-state authored SFT; no validation selection'
    value['corrective_reduction_study']=dict(arm=arm,ready_sha256=s.sha(s.ROOT/'READY.json'),terminal_weight=.05)
