...
s.check(s.ROOT/'launch.py','e3f01064d9a529200340783c1a9e1cd116a44ce3ab0da26350a2ec8b5daec50c')
...
def execute(output):
    started=time.time();ready=r.verify_corrected()
    ...
    s.write(output/'RUN.json',dict(identity=ready['identity'],ready_sha256=s.sha(s.ROOT/'READY_V2.json'),started_epoch=started,
    ...
    try:
        ...
        for arm in s.phase_order():
            ...
            try:
                ...
                argv=[str(s.NATIVE),str(s.ROOT/'readout_v2.py'),'--weight',arm,'--training',str(b.training_source(arm)),
...
if __name__=='__main__':
    ...
    if a.command=='verify':print({'identity':r.verify_corrected()['identity'],'gpu_calls':0})
    else:
