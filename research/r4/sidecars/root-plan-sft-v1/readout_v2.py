...
import functools
s.check(s.OLD/'readout.py',s.PINS['readout.py'])
ORIGINAL_READY_SHA='6c511ac8ef9aec636175a79b9a76a2b776999b24a9e2e64348fbb90d40ef2a4a'

@functools.lru_cache(maxsize=1)
def verify_corrected():
    s.check(s.ROOT/'READY.json',ORIGINAL_READY_SHA)
    original=s.verify();corrected=s.read(s.ROOT/'READY_V2.json')
    if s.digest({k:v for k,v in corrected.items() if k!='identity'})!=corrected['identity']:
        raise ValueError('corrected READY identity')
    if corrected['scientific_identity']!=original['identity'] or corrected['original_ready_sha256']!=ORIGINAL_READY_SHA:
        raise ValueError('unchanged science identity')
    if corrected['input_sha256']!=original['input_sha256']:
        raise ValueError('prelaunch correction changed scientific inputs')
    for path,want in corrected['source_sha256'].items():
        if path in original['source_sha256']:
            if want!=original['source_sha256'][path]:raise ValueError('old frozen source changed')
        else:s.check(path,want)
    return original


async def collect(args):
    import httpx
    ready = verify_corrected()
    ...
    plan, prompts = s.read(s.ROOT / 'prepared-v2/PLAN.json'), s.read(s.ROOT / 'prepared-v2/PROMPTS.json')
    ...
    s.write(args.output.parent / 'READOUT_BINDING.json', dict(identity=ready['identity'], arm=args.weight,
        ...
        selected=chosen, plan_sha256=s.sha(s.ROOT / 'prepared-v2/PLAN.json'),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight', choices=('unchanged', 'canonical', 'filter_first'), required=True)
