"""Bind V2 ownership checks to the attempt-002 wrapper identity."""
...
SERVICE = s.ROOT / "service_wrapper_v3.py"
...
def claim_source():
    ...
    node = next(n for n in ast.parse(text_source).body
                if isinstance(n, ast.FunctionDef) and n.name == "claim_service")
    text = ast.get_source_segment(text_source, node)
...
def install(suite):
    ...
    exec(compile(claim_source(), str(SOURCE) + ":free-id-attempt002-wrapper", "exec"),
         suite.life.__dict__)
