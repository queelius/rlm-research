"""Attempt-002 owner using the proven dual-LoRA startup dependency."""
import argparse,importlib.util,json,sys
from pathlib import Path
import checkpoint_repair as checkpoint
import study_repair as study
SOURCE=study.SIDE/'openai-mrcr-long-transfer-eval-v1'/'owner.py';text=SOURCE.read_text()
old={n:sys.modules.get(n) for n in ('study','checkpoint')};sys.modules.update(study=study,checkpoint=checkpoint)
try:
    spec=importlib.util.spec_from_loader('balanced32_owner_repair_source',loader=None);source=importlib.util.module_from_spec(spec);source.__file__=str(SOURCE)+':balanced32-repair';exec(compile(text,source.__file__,'exec'),source.__dict__)
finally:
    for n,v in old.items():
        if v is None:sys.modules.pop(n,None)
        else:sys.modules[n]=v
source.STAGES={'cp32':{'arm':'cp32','output':study.ROOT/'outputs/cp32-002'},'lr1e4':{'arm':'lr1e4','output':study.ROOT/'outputs/lr1e4-002'}}
