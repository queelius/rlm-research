...
source=path.read_text();changes=(('mnli-exact-tag32-collect','mnli-new-context-alien48-collect'),("terminal['planned']!=32 or terminal['recorded']!=32","terminal['planned']!=48 or terminal['recorded']!=48"),("planned=32,elapsed_seconds","planned=48,elapsed_seconds"))
for old,new in changes:
    if source.count(old)!=1:raise ValueError('owner seam changed: '+old)
    source=source.replace(old,new)
module=types.ModuleType('new_context_alien_owner');module.__file__=str(path);sys.modules[module.__name__]=module
