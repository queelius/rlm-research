"""Attempt-003 metadata gate; collection stays in the qualified source namespace."""
...
def verify_prepared():
    campaign = base.read(ROOT / "CAMPAIGN_V5.json")
    ...
    if ready["identity"] != campaign["identity"] or ready["campaign_sha256"] != base.sha(ROOT / "CAMPAIGN_V5.json"):
        raise ValueError("READY V5 identity")
    if ready["attempt"] != str(ATTEMPT) or campaign["attempt"] != str(ATTEMPT):
        raise ValueError("attempt-003 binding")
