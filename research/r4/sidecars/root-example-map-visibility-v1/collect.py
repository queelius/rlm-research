...
async def collect(args):
    ...
    terminal = dict(planned=32, recorded=len(results), available=sum(r.get('reward') is not None for r in results.values()),
                    ...
                    cells={f'example_{int(example)}_inline_{int(inline)}': [r['id'] for r in plan
                           if (r['example'], r['inline']) == (example, inline)]
                           for example in (False, True) for inline in (False, True)},
