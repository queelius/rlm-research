...
def prepare():
    ...
    for row in rows:
        ...
        prompt = protocol.prompt(st.prior.prompt(context, row['family']), row, maps[context['id']]['file_text'].encode())
    ...
    s.write(s.ROOT / 'PREPARED.json', dict(master_seed=protocol.MASTER,
            seed_collision_scan='rg981342[0-9]{3} before source creation returned1/no matches in sidecars+ideas excluding outputs and qualification',
            contexts=4, endpoints=32, native_map_acquisitions=0, reused_acquisitions=4, paired_blocks=8,
            data_ready_sha256=s.sha(s.SOURCE / 'DATA_READY.json'),
            max_prompt_tokens=max(len(p['token_ids']) for p in prompts), gpu_calls=0))


def seal():
    previous = s.read(s.MAP_SOURCE / 'READY.json')
    if s.sha(s.MAP_SOURCE / 'READY.json') != '5aca1018840158985491f12f3cb9e39d6956b8b014db3c6dd04a3f1efc5b9b98':
        raise ValueError('accepted supplied-map science READY changed')
    ...
    for path in [s.MAP_SOURCE / 'READY.json', s.MAP_SOURCE / 'OWNER_READY.json', s.SOURCE / 'DATA_READY.json',
    ...
    ready = dict(status='CPU_READY_FOR_MAIN_ACCEPTANCE', source_sha256=sources,
                 ...
                 child_sha256=s.source().CHILD_SHA, endpoints=32, acquisitions=0, reused_acquisitions=4,
