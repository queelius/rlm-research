...
CPU_QUALIFICATION.runtime-order-v2.json validates all24+32 actual protocol bodies, compiles9 representation schemas and1 rotation schema against the frozen tokenizer, and records every rendered prompt hash. Inputs max2069/1566 tokens respectively, so caps fit the8192 context limit. It verifies installed CPU protocol/compiler support, not live enforcement.

The authoritative frozen inputs are SPEC-REPRESENTATION.runtime-order-v2.json and SPEC-ROTATION.runtime-order-v2.json; the bind command defaults to these. Pre-READY v1 specs/qualification and archived_pre_ready source copies are retained but NOT runnable evidence: initial CPU rendering used native tool dictionary insertion order, whereas saved runtime specs sort JSON keys. That changed physical tool-token ordering without changing any system/user text or tool field values. V2 constructs the same sorted contract ordering before qualification and after reload, preserving the already prepared actual runtime requests. No data, treatment, cap, weight, seed or grammar changed, and no v1 model call occurred.
