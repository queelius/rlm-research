"""24 direct-source native interface roots, with physical-cost and NULL ledgers."""
import argparse
import asyncio
import json
import os
...
async def run(args):
    ...
    s.write(args.output / 'RUN.json', dict(identity=ready['identity'], endpoint=endpoint, started_epoch=started,
                                          deadline=args.deadline, root_deadline=root_deadline, planned=24, acquisitions=0))
    ...
    packages = {(w['id'], arm): p.evidence(w, arm, []) for w in worlds for arm in p.REPRESENTATIONS}
    ...
    s.write(args.output / 'SUMMARY.json', dict(cells=cells, rows=values, acquisitions=[], world_clusters=4,
                 costs='Physical native root/optional-child requests and observed tokens/time; no acquisition; provider billing unknown'))
    status = dict(planned=24, recorded=len(values), available=sum(r['available'] for r in values), correct=sum(r['reward'] == 1 for r in values),
                  physical_root_pipeline_attempts=sum(r.get('physical_attempts', 0) for r in values),
                  complete=len(values) == 24, all_native_finals_available=all(r['available'] for r in values), elapsed_seconds=time.time()-started)
