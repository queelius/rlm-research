...
PROOF = s.ROOT / 'qualification-002/RESULT.json'
...
def inputs():
    ...
    if not proof['passed'] or proof['native_calls'] != 7 or proof['actual_child_calls'] != 1:
        raise ValueError('actual three-arm native proof required')
    ...
    receipt=dict(source_worlds=str(p.SOURCE/'WORLDS.json'),source_worlds_sha256=s.sha(p.SOURCE/'WORLDS.json'),
        ...
        seed_check='Before new protocol creation, rg word-boundary981397[0-9]{3} on sidecar PLAN/SEED/protocol/study/READY returned no matches',
        order='six permuted arm orders plus first two repeated, pre-outcome shuffled eight blocks',
    ...
    print(dict(inputs=24, maximum_initial_plus_output=max(receipt['prefix_lengths'].values())+2560))


def qualify():
    command=[str(s.NATIVE),'-m','unittest','-v','test_interface','test_owner','test_collect']
    started=time.time()
...
def seal():
    ...
    source=dict(s.read(p.SOURCE/'READY_V2.json')['source_sha256'])
    ...
    ready=dict(status='CPU_READY_FOR_MAIN_ACCEPTANCE',source_sha256=source,model=s.MODEL,adapter=None,
        planned_root_endpoints=24,planned_acquisitions=0,exposed_world_clusters=4,arms=list(p.REPRESENTATIONS),
        ...
        native_fixture=str(PROOF),focused_tests=10,seed_master=p.MASTER,
