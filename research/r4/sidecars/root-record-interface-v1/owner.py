"""MAIN-only released-base lifecycle; exact namespace and inclusive 1200s envelope."""
import argparse
import os
...
def execute(output):
    ...
    runner = suite(); started = time.time(); work = started + 1080; owned = started + 1170
    ...
    s.write(output / 'OWNER_RUN.json', dict(identity=ready['identity'], ready_sha256=s.sha(s.READY_PATH),
        started_epoch=started, work_deadline=work, owned_deadline=owned, outer_seconds=1200, gpu=gpu, **private))
    def expired(*_): raise TimeoutError('owned1170s deadline or MAIN termination')
    ...
    try:
        ...
        runner.command(stage, 'record-interface-collect', argv, max(.001, work-time.time()), work)
        terminal = s.read(output / 'rollout/STATUS.json')
        if terminal['planned'] != 24 or terminal['recorded'] != 24:
            raise ValueError('collector must retain24 planned slots')
    ...
    result = dict(complete=error is None and released, error=error, release_error=release_error,
                  released=released, collector_status=terminal, planned=24,
