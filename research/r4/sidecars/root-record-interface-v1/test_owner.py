...
class OwnerTests(unittest.TestCase):
    def test_actual_owner_start_failure_keeps24_slots_and_cleanup_budget(self):
        import os
        import tempfile
        ...
        with tempfile.TemporaryDirectory(prefix='interface-owner-failure-') as directory:
            ...
            self.assertEqual(len(original_read(output/'PLANNED_NULL_ENDPOINTS.json')),24)
    ...
    def test_actual_collector_cli_is_bound_to_new_attempt(self):
        ...
        with self.assertRaises(ValueError):
            owner.validate_argv([x.replace('root-record-interface-v1', 'root-partition-final-interface-v1') for x in argv])
