"""Authored CPU-only native three-interface/file/root/child/final qualification."""
import argparse
import asyncio
import hashlib
import json
...
async def qualify(output):
    ...
    world = p.worlds()[0]
    rows = [dict(id='cpu-interface-'+arm, python=True, seed=981397901, representation=arm) for arm in p.REPRESENTATIONS]
    ...
    try:
        ...
        if counts != {'cpu-interface-T': 2, 'cpu-interface-F': 3, 'cpu-interface-I': 2}:
        ...
        for row in rows:
            ...
            if row['representation'] == 'F' and 'INTERFACE_CHILD_SENTINEL' not in observations[0]:
