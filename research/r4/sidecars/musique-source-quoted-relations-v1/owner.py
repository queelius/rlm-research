"""Current explicit collector binding; qualified V3 lifecycle and cached baseline model."""
import argparse
import json
...
def implementation():
    ...
    for before,after in (("'planned_calls': 72","'planned_calls': 24"),("'planned_final_slots': 24","'planned_final_slots': 12")):
        assert text.count(before)==1;text=text.replace(before,after)
    module=ModuleType('source_quoted_v3_owner');module.__file__=str(study.PRIOR/'owner.py')+':24-call-12-new-final'
