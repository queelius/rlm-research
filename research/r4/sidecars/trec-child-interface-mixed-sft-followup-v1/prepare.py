"""Build the deterministic half-full6/half-ABO training stream and two-panel readout."""
...
def ordered(rows, role, key):
    # Preserve the exact predecessor contexts/order; only the interface schedule changes.
    return sorted(rows, key=lambda row: s.digest([
        "trec-child-interface-sft-followup-v1", role, row[key]
    ]))
...
def training_rows(closure):
    ...
    for context in range(96):
        ...
        for interface in candidates:
            ...
            candidates[interface].append(row)
    mixed = [candidates["full6" if context % 12 < 6 else "abo"][context]
             for context in range(96)]
    return {"mixed": mixed}
...
def write_inputs(output, closure=None):
    ...
    provenance = {
        "schema": "trec-child-interface-mixed-sft-followup-provenance-v1",
        ...
        "train_contexts": 96,
        "train_interface_contexts": {"full6": 48, "abo": 48},
        "train_pair_interface_contexts": {
            str(list(pair)): {"full6": 8, "abo": 8} for pair in s.TRAIN_PAIRS
        },
