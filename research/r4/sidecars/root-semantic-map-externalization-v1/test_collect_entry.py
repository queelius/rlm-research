...
def test_actual_collector_gates_six_roots_per_failed_source(tmp_path,monkeypatch):
    import httpx
    ...
    class Client:
        ...
        async def post(self,url,**kwargs):
            ...
            assert body['model']==binding['fixed_child'] and 'tools' not in body and 'structured_outputs' in body
            reply=json.dumps({r['id']:'entity' for r in public[index]['records']}) if index<7 else '{}'
    ...
    assert len(sent)==8 and len(rows)==48 and len(attempted)==42
    ...
    assert c.ledger(output)['sources']['physical_request_attempts']==8
