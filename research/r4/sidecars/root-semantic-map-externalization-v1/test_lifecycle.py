...
def test_actual_owner_to_qualified_service_entry_before_intercepted_model_launch(tmp_path):
    ...
    with s.aliases({'study':s,'protocol':p}):owner=s.load('contract_evidence_lifecycle_test_owner',s.ROOT/'sm_owner.py',s.sha(s.ROOT/'sm_owner.py'))
    ...
    def popen(argv,**kwargs):
        ...
        if len(spawned)==1:
            ...
            assert len(s.read(output/'PLANNED_NULL_ACQUISITIONS.json'))==8
