"""One actual dual-adapter service, exact attempt and inclusive2400 envelope."""
import argparse
import functools
import os
...
def dependencies():
    old=s.load('sm_qualified_lifecycle_owner',s.CE/'owner.py','8967d888d38428c273d2b60a993183f28887a3ca9b2ee67eb331e9ac7bb28d03',{'study':s,'protocol':p})
...
def collector_argv(output,deadline):
    return [str(s.NATIVE),str(s.ROOT/'sm_collect.py'),'--binding',str(output/'service/BINDING.json'),'--endpoint',str(output/'service/service/endpoint-original.json'),'--output',str(output/'rollout'),'--deadline',str(deadline)]
def execute(output):
    started=time.time();work=started+2220;owned=started+2370;check_output(output);s.runtime();ready=s.verify()
    ...
    s.write(output/'PLANNED_NULL_ENDPOINTS.json',planned);s.write(output/'PLANNED_NULL_ACQUISITIONS.json',[dict(coordinate=r,available=False,physical_request_attempt=False) for r in s.read(s.ROOT/'inputs/ACQUISITION_PLAN.json')])
    s.write(output/'OWNER_RUN.json',dict(identity=ready['identity'],ready_sha256=s.sha(s.ROOT/'READY.json'),started_epoch=started,work_deadline_epoch=work,owned_deadline_epoch=owned,outer_seconds=2400,cleanup_seconds=150,outer_margin_seconds=30,source_cap_seconds=300,root_cap_reserved_seconds=1500,closure_reserved_seconds=120,startup_seconds=180,caps_not_waits=True,credential_present=True,credential_value_logged=False,gpu=gpu,no_training=True))
    def expired(_sig,_frame):raise TimeoutError('owned2370 inclusive or MAIN termination')
    previous={sig:signal.signal(sig,expired) for sig in (signal.SIGINT,signal.SIGTERM,signal.SIGALRM)};signal.setitimer(signal.ITIMER_REAL,remaining(owned,2370))
    stage=output/'service';stage.mkdir();active=stage;error=None;release_error=None
    try:
        suite.start_service(stage,s.binding(),time.time()+remaining(work-1920,180))
        suite.command(stage,'semantic-map48',collector_argv(output,work),remaining(work,2220),work)
    ...
    try:
        import sm_collect as collector
        for row in planned:
            path=Path(row['path'])
            if path.exists():
        ...
        result=dict(complete=error is None and release_error is None,error=error,release_error=release_error,readout_inventory=planned,planned=48,planned_acquisitions=8,elapsed_seconds=time.time()-started,active_unreleased_service=str(active) if active else None,no_training=True,no_retry=True,main_owns_gpu_and_lock=True,orchestrator_complete_does_not_imply_native_final=True,cost_ledger_sha256=s.sha(output/'COST_LEDGER.json'))
