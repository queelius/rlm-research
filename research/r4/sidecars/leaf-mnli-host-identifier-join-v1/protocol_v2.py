...
ARMS=tuple(r+'_'+o for r in ('wrong','alien','aligned') for o in ('tag_first','labels_only'))
...
def schema(context,arm):
    if arm.endswith('labels_only'):
        return {'json':dict(type='array',minItems=48,maxItems=48,items={'type':'string','enum':list(LABELS)})}
    rows=[]
...
def plan():
    ...
    for ci in sorted(range(8),key=lambda i:s.digest([MASTER,'dispatch',i])):
        ...
        for pos,arm in enumerate(arms):
            row=dict(context_index=ci,repeat=0,seed=983626101+ci,arm=arm,
                     decoder='exact_requested_tag',size=48,block=ci,pair_position=pos,dispatch_order=len(rows))
            row['id']=s.digest(['mnli-host-identifier-join48-v1',row]);rows.append(row)
    return rows
def request(context,row):
    instruction=EXPLICIT.format(order='tag then label')
    if row['arm'].endswith('labels_only'):
        before,rest=instruction.split('Return exactly 48 JSON objects',1)
        _,after=rest.split('Input records ',1)
        instruction=before+("Return exactly 48 strings in one JSON array, in displayed record order. Each string must be exactly entailment, neutral, or contradiction. Return labels only; ordinary code will attach each record's requested_tag afterward. No additional text.\nInput records ")+after
    prompt=instruction+s.serialize(visible_records(context,row['arm']))
