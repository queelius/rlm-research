"""Additive MAIN-owned launcher; original diagnostic READY and source stay frozen."""
import argparse
import functools
import os
import signal
...
SCIENTIFIC_SHA = '5aca1018840158985491f12f3cb9e39d6956b8b014db3c6dd04a3f1efc5b9b98'


def credential_preflight():
    credential = s.load('diagnostic_credential_preflight', RUNTIME / 'credential_preflight.py',
...
def dependencies():
    ...
    with s.source().aliases({'study': original}):
        launcher = s.load('diagnostic_qualified_owned_dependencies', original.ROOT / 'launch.py',
                          '62d7d0049b5e3303f6e68247038290c7cde0d74e3394b4856fea0e3e138d6b46')
    suite = launcher.dependencies()
    previous = list(sys.path)
    sys.path.insert(0, str(RUNTIME))
    try:
        adapter = s.load('diagnostic_actual_service_lifecycle', RUNTIME / 'lifecycle_adapter.py',
...
def verify():
    if s.sha(s.ROOT / 'READY.json') != SCIENTIFIC_SHA:
...
def execute(output):
    ...
    stage.mkdir()
    s.write(output / 'OWNER_RUN.json', dict(owner_identity=ready['identity'], scientific_ready_sha256=SCIENTIFIC_SHA,
    ...
    try:
        ...
        argv = [str(s.NATIVE), str(s.ROOT / 'collect.py'), '--binding', str(stage / 'BINDING.json'),
                ...
                '--deadline', str(work)]
        suite.command(stage, 'diagnostic-collect', argv, 1200, work)
    ...
    result = dict(complete=error is None and released, error=error, release_error=release_error,
                  released=released, collector_terminal=terminal, planned=32, acquisitions=4,
...
def seal():
    ...
    value = dict(status='CPU_READY_OWNED_SUCCESSOR_FOR_MAIN_ACCEPTANCE', source_sha256=sources,
                 ...
                 service_readiness_cap_seconds=180, shared_clock='owner start through service startup plus4 acquisitions and32 endpoints',
                 argv=[str(s.NATIVE), str(s.ROOT / 'owner.py'), 'run', '--output', str(ATTEMPT)],
                 verify_argv=[str(s.NATIVE), str(s.ROOT / 'owner.py'), 'verify'],
                 output_nesting='owner at outputs/attempt-001; service at owned-service; unchanged collector at rollout',
