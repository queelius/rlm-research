"""Actual optional module load, failed action, reload and native final; CPU authored only."""
import argparse
import asyncio
import json
import os
...
async def qualify(output):
    ...
    rows=[{**row,'id':'CL_CPU_'+str(i)} for i,row in enumerate(value['PLAN.json'][:2])];query=value['QUERIES.json'][rows[0]['task_name']]
    ...
    loader_first=('import source_state\nsnapshot = source_state.load()\nsnapshot["records"][0]["weight"] = -999\nsnapshot["predictions"].clear()\nraise NameError("AUTHORED_CPU_RELOAD_CHECK")')
    loader_code=code.replace('records = json.load(open("records.json"))\nlabels = json.load(open("labels.json"))',
        'reloaded = source_state.load()\nrecords = reloaded["records"]\nlabels = reloaded["predictions"]\nassert records[0]["weight"] == '+repr(context['records'][0]['weight'])+'\nassert labels == '+repr(labels))
    ...
    endpoint=dict(url=f'http://127.0.0.1:{site._server.sockets[0].getsockname()[1]}/v1',model=root,renderer_model=str(s.qnative().stack().prior.BASE),api_key_env='CL_CPU_PROVIDER_KEY')
    before={k:os.environ.get(k) for k in ('STRICT_RLM_CALIBRATION_API_KEY','CL_CPU_PROVIDER_KEY')}
        ...
        with n.installed(interface,binding,output,rows,{context['id']:context},expectations):
            ...
            async with env.serving():
                for row in rows:
                    ...
                    assert c.terminal(trace,capture,expected_scalar)['reward']==1 and len(roles)==(3 if row['representation']=='FILE' else 4)
                    assert sum(r['depth']>0 for r in roles.values())==1
                    attestation=trace['info']['canonical_loader_setup'];assert attestation['plain_query']==query['question'];attestations.append(attestation['file_sha256'])
                    if row['representation']=='LOADER':
                        assert attestation['module_sha256'] and any('AUTHORED_CPU_RELOAD_CHECK' in (node['message'].get('content') or '') for node in trace['nodes'] if node['message'].get('role')=='tool')
                    else:assert attestation['module_sha256'] is None
                    assert any(node['message'].get('role')=='tool' and str(expected_scalar) in node['message'].get('content','') for node in trace['nodes'])
        assert len(calls)==7 and attestations[0]==attestations[1]
        s.write(output/'RESULT.json',dict(passed=True,actual_native_episodes=2,native_calls=7,child_calls=2,root_calls=5,common_file_sha256=attestations[0],actual_model_calls=0,gpu_calls=0,model_service_calls=0,all_first_prefixes_and_final_branches_verified=True,root_role_unchanged=True,authored_file_code=code,authored_loader_first=loader_first,authored_loader_reload=loader_code,optional_loader_mutation_then_error_then_reload_verified=True,elapsed_seconds=time.time()-started,source_sha256={str(path):s.sha(path) for path in s.ROOT.glob('*.py')}))
        print(dict(passed=True,cpu_native_calls=7,elapsed_seconds=time.time()-started))
