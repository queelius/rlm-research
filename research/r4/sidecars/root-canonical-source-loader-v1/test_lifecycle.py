...
def test_actual_owner_to_qualified_service_entry_before_intercepted_model_launch(tmp_path):
    assert Path(__file__).with_name('cl_owner.py').exists(), 'owner service entry not implemented'
    import cl_study as s
    import cl_protocol as p
    with s.aliases({'study':s,'protocol':p}):owner=s.load('contract_evidence_lifecycle_test_owner',s.ROOT/'cl_owner.py',s.sha(s.ROOT/'cl_owner.py'))
    ...
    def popen(argv,**kwargs):
        spawned.append(argv)
        if len(spawned)==1:
            rows=s.read(output/'PLANNED_NULL_ENDPOINTS.json');assert len(rows)==48 and all(x['reward'] is None for x in rows)
            assert len(s.read(output/'REUSED_SOURCE_INVENTORY.json'))==8
    ...
    assert binding==s.binding() and endpoint['adapter']['model_sha256']=='857a7ce6907c759a8c1b478eb53029d3b700b81c3094d8edb3394e52def9fcb6'
