"""Seal the additive V2 inherited-interface and checkpoint-provenance repair."""
...
def main():
    ...
    if study.READY.exists() or any(path.exists() for path in owner.STAGES.values()):
        raise FileExistsError("V2 READY or output already exists")
    ...
    plan = owner.plan()
    ready = {
        "schema": "mrcr-token-tis-held16-three-arm-cpu-ready-v2",
        "status": "CPU_READY_CONDITIONAL_ON_TWO_INDEPENDENT_STEP1_BRANCHES_V2",
        "created_epoch": time.time(),
        "supersedes_unlaunched_ready": str(study.ROOT / "CPU_READY.json"),
        "superseded_ready_sha256": study.sha(study.ROOT / "CPU_READY.json"),
        ...
        "metrics": {
            ...
            "diagnostic": ["raw official similarity", "similarity>=0.90", "shaped reward", "availability", "native calls and tokens"],
    ...
    print(json.dumps({"ready": str(study.READY), "sha256": study.sha(study.READY), "identity": ready["identity"]}, sort_keys=True))
