"""V3 fixed held16 owner with the actual repaired collector command."""
...
STAGES = {
    "base": study.ROOT / "outputs-v3/base-001",
    "lr1e-5": study.ROOT / "outputs-v3/lr1e-5-001",
    "lr1e-4": study.ROOT / "outputs-v3/lr1e-4-001",
}
...
def _load_source():
    ...
    text = text.replace(before, 'str(study.ROOT / "collect_v3.py")')
    previous = {name: sys.modules.get(name) for name in ("study", "checkpoint")}
    sys.modules["study"] = study
    sys.modules["checkpoint"] = checkpoint
    try:
        spec = importlib.util.spec_from_loader("token_tis_held_v3_owner_source", loader=None)
        module = importlib.util.module_from_spec(spec)
        module.__file__ = str(path) + ":token-tis-v3"
...
def plan():
    return {
        "schema": "mrcr-token-tis-held16-three-arm-plan-v3",
        "stage_argv": {
            stage: [
                str(study.NATIVE),
                str(study.ROOT / "owner_v3.py"),
                "run",
                "--stage",
                stage,
                "--output",
        ...
        "collector": str(study.ROOT / "collect_v3.py"),
    }
