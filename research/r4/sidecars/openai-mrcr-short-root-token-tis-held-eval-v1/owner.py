"""Three fixed held16 owners using the already qualified raw MRCR evaluation path."""
...
def _source():
    ...
    try:
        spec = importlib.util.spec_from_file_location("token_tis_held_source_owner", path)
...
def plan():
    return {
        "schema": "mrcr-token-tis-held16-three-arm-plan-v1",
        "stage_argv": {
            stage: [
                str(study.NATIVE),
