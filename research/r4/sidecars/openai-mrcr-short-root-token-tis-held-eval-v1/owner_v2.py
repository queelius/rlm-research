"""V2 held16 owners, rebinding the inherited command to collect_v2.py."""
...
STAGES = {
    "base": study.ROOT / "outputs-v2/base-001",
    "lr1e-5": study.ROOT / "outputs-v2/lr1e-5-001",
    "lr1e-4": study.ROOT / "outputs-v2/lr1e-4-001",
}
...
def _load_source():
    ...
    text = text.replace(before, 'str(study.ROOT / "collect_v2.py")')
    previous = {name: sys.modules.get(name) for name in ("study", "checkpoint")}
    sys.modules["study"] = study
    sys.modules["checkpoint"] = checkpoint
    try:
        spec = importlib.util.spec_from_loader("token_tis_held_v2_owner_source", loader=None)
        module = importlib.util.module_from_spec(spec)
        module.__file__ = str(path) + ":token-tis-v2"
...
def plan():
    return {
        "schema": "mrcr-token-tis-held16-three-arm-plan-v2",
        "stage_argv": {
            stage: [
                str(study.NATIVE),
        ...
        "collector": str(study.ROOT / "collect_v2.py"),
    }
