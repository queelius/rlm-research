"""Seal final additive V4 after exercising the real inherited boundaries."""
...
def main():
    ...
    if study.READY.exists() or any(path.exists() for path in owner.STAGES.values()):
        raise FileExistsError("V4 READY or output already exists")
    ...
    local_names = (
        "study_v4.py", "checkpoint_v4.py", "collect_v4.py", "owner_v4.py",
        "prepare_v4.py", "test_eval_v4.py", "study_v3.py", "checkpoint_v3.py",
        "collect_v3.py", "owner_v3.py", "prepare_v3.py", "test_eval_v3.py",
        "CPU_READY_V3.json", "study_v2.py", "checkpoint_v2.py", "collect_v2.py",
        "owner_v2.py", "prepare_v2.py", "test_eval_v2.py", "REPAIR_V2.md",
        "CPU_READY_V2.json", "CPU_READY.json", "study.py", "checkpoint.py",
        "collect.py", "owner.py", "prepare.py", "test_eval.py", "DESIGN.md", "RUNBOOK.md",
    )
    paths = [study.ROOT / name for name in local_names]
    paths.extend([training_ready_path, study.TRAIN_INPUTS, study.SOURCE_EVAL / "CPU_READY.json",
                  study.SOURCE_EVAL / "study.py", study.SOURCE_EVAL / "checkpoint.py",
                  study.SOURCE_EVAL / "collect.py", study.SOURCE_EVAL / "owner.py"])
    ...
    ready = {
        "schema": "mrcr-token-tis-held16-three-arm-cpu-ready-v4",
        "status": "CPU_READY_CONDITIONAL_ON_TWO_INDEPENDENT_STEP1_BRANCHES_V4",
        "created_epoch": time.time(),
        "supersedes_unlaunched_ready": str(study.ROOT / "CPU_READY_V3.json"),
        "superseded_ready_sha256": study.sha(study.ROOT / "CPU_READY_V3.json"),
        "v3_reason": "version-wrapper test assertions named the V2 test/status; no GPU launch occurred",
        ...
        "checkpoint_gate": {"both_independent_step1_commits": True,
            "result_state_commit_links": True, "initial_tensor_identity": True,
            "initial_rng_and_saved_gradient_hashes": True, "result_linked_10x_relation": True,
            "both_postupdate_likelihood_inventories": True, "checkpoint_selection": False},
        "sampling": {"temperature": 0.5, "max_tokens": 2048, "max_total_turns": 6,
                     "retries": 0, "workers": 4},
        "metrics": {"primary": "paired raw exact",
