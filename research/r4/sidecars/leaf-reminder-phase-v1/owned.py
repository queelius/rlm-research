"""Private870-second owned lifecycle; fixed c32, MAIN alone launches."""
import ast
import hashlib
...
EDITS=[('identity_counter','reminder_phase',5),('1080','750',1),('1200','870',3),('all96','all192',1),('phase96-run','phase192-run',1),('if __name__ == "__main__":','if False:',1)]
for before,after,count in EDITS:
    if source.count(before)!=count:raise ValueError('owned edit seam changed: '+before)
    source=source.replace(before,after)
ADAPTED_SHA256=hashlib.sha256(source.encode()).hexdigest();exec(compile(source,str(SOURCE_PATH)+':phase192-private','exec'),globals())
_load_suite=load_suite
def load_suite():
    suite=_load_suite()
    if sha(OBSERVER_PATH)!=OBSERVER_SHA:raise ValueError('absence observer changed')
    ...
    original=suite.life.v1.process_identity;suite.life.v1.process_identity=lambda pid:scope['observe_or_absent'](original,pid)
    return suite
