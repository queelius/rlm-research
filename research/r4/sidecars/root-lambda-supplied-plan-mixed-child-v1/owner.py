"""Bounded MAIN-only owner for the forty-call mixed-child supplied-plan bridge."""
import argparse
import os
...
def collector_argv(stage, output, deadline):
    return [str(s.NATIVE), str(s.ROOT / "collect.py"), "--endpoint",
            str(stage / "service/endpoint-original.json"), "--output", str(output),
            "--deadline", str(float(deadline))]


def execute(output):
    output = Path(output); started = time.time(); work = started + 1080; owned = started + 1170
    ...
    s.write(output / "OWNER_RUN.json", {"identity": ready["identity"], "started_epoch": started,
            "work_deadline": work, "owned_deadline": owned, "outer_seconds": 1200,
            "startup_cap": 180, "collector_harvest_reserve": 30, "release_cap": 90,
            "finalize_reserve": 30, "outer_margin": 30, "planned_child_calls": 40,
            "root_model_calls": 0, "credential_present": True, "gpu": gpu})
    def expired(sig, frame):
        if sig in (signal.SIGINT, signal.SIGTERM): raise MainTermination("MAIN cancellation")
        raise TimeoutError("bounded mixed-child supplied-plan cap")
    handlers = {sig: signal.signal(sig, expired) for sig in
    ...
    try:
        ...
        suite.command(stage, "supplied-plan40", collector_argv(stage, output / "rollout", work),
                      max(.001, work - time.time()), work)
    except BaseException as error:
        errors.append({"stage": "work", "type": type(error).__name__, "message": str(error)})
    finally:
        release_started = time.time()
        signal.setitimer(signal.ITIMER_REAL,
                         max(.001, min(owned - 30, time.time() + 90) - time.time()))
        ...
        module = collect.implementation(); ledger = module.harvest(output / "rollout", plan)
        s.write(output / "COST_LEDGER.json", {key: value for key, value in ledger.items()
                                               if key != "rows"})
        result = {"identity": ready["identity"], "complete": not errors, "released": not active,
                  "active_unreleased_service": str(stage) if active else None,
                  "error": errors or None, "inventory": ledger["rows"], "planned": 40,
                  "physical": ledger["cost"], "elapsed_seconds": time.time() - started,
                  "release_started_epoch": release_started, "ended_epoch": time.time(),
                  "root_model_calls": 0, "no_retry": True}
...
def main():
    ...
    if args.command == "verify": print(s.verify()["identity"])
        ...
        result = execute(args.output); print({"complete": result["complete"],
