"""Bounded MAIN-only owner for the forty-call supplied-plan ceiling."""
import argparse
import os
...
def execute(output):
    output = Path(output); started = time.time(); work = started + 1650; owned = started + 1770
    ...
    s.write(output / "OWNER_RUN.json", {"identity": ready["identity"], "started_epoch": started,
            "work_deadline": work, "owned_deadline": owned, "outer_seconds": 1800,
    ...
    def expired(sig, frame):
        ...
        raise TimeoutError("bounded supplied-plan cap")
