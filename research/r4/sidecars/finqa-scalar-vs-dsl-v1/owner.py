"""Qualified B05V3 lifecycle; trusted owner source only, never model-output execution."""
import argparse
import json
import types
import collect
...
def implementation():
    with study.aliases({"runner_study":study.source},study.SOURCE):
        v3 = study.load("finqa_source_owner_v3",study.SOURCE / "runner_owner_v3.py")
    _original,repaired = v3.owner_sources()
    for before,after in (('"planned_coordinates": 64','"planned_coordinates": 32'),('"planned_physical_cap": 64','"planned_physical_cap": 32')):
        assert repaired.count(before)==1
        repaired = repaired.replace(before,after)
    module = types.ModuleType("finqa_native_v3_owner")
    module.__file__ = __file__
    module.lifecycle = v3.lifecycle_module()
    # This compiles only pinned, inspected owner infrastructure; science.py never
    # calls Python eval/exec and no generated program enters this code path.
