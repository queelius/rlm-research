...
@pytest.mark.parametrize('variant',['original','counterfactual'])
def test_actual_two_stage_native_prefix_and_execution(tmp_path,monkeypatch,arm,variant):
    assert Path(__file__).with_name('inputs').exists(),'actual frozen96 native inputs absent'
    import cf_study as s
    import cf_collect as c
    from aiohttp import web
    monkeypatch.setenv('STRICT_RLM_CALIBRATION_API_KEY','CPU_FIXTURE_NOT_CREDENTIAL');monkeypatch.setenv('COUNTER_CARD_CPU_KEY','CPU_FIXTURE_NOT_CREDENTIAL')
    async def fixture():
        operator='maximum_weight' if variant=='original' else 'threshold_users'
        row=next(r for r in s.read(s.ROOT/'inputs/FREE_PLAN.json') if r['operator']==operator and r['card_arm']==arm and r['variant']==variant and (variant=='original' or r['threshold']!=5))
        ...
        totals=[sum(r['weight'] for r in context['records'] if r['user']==u and labels[r['id']]==target) for u in users]
        expected=max(totals) if operator=='maximum_weight' else sum(v>row['threshold'] for v in totals)
        ...
        if operator=='threshold_users':code2=code2.replace('max(totals.values(),default=0)','sum(value>'+str(row['threshold'])+' for value in totals.values())')
        async def provider(request):
            ...
            return web.json_response(dict(request_id='CF_CPU_'+str(len(roots)+len(children)),usage=dict(prompt_tokens=len(body['token_ids']),completion_tokens=len(ids)),choices=[dict(token_ids=ids,finish_reason='stop',logprobs=dict(content=[dict(token=f'token_id:{v}',logprob=-.5) for v in ids]))]))
        ...
        descriptor=dict(host='127.0.0.1',port=site._server.sockets[0].getsockname()[1],api_key_env='COUNTER_CARD_CPU_KEY')
        try:
