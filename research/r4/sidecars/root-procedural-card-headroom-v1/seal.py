"""Freeze focused CPU qualification and the exact approved48-root package."""
import os
import subprocess
import time
...
def main():
    ...
    s.write(s.ROOT/'CPU_REPORT.json',dict(argv=argv,returncode=result.returncode,stdout=result.stdout,stderr=result.stderr,elapsed_seconds=time.time()-begin,scientific_model_calls=0,gpu_calls=0,authored_native_root_calls=6,authored_native_child_calls=2,actual_owner_service_wrapper_intercepted=True,synthetic_fixture_not_scientific_evidence=True,earlier_red_receipts=['RED_TASK.xml','RED_OWNER.xml','RED_INPUT_NATIVE.xml']))
    ...
    for name in ('2026-09-10-recuris-rlmopt-next-levers.md','2026-09-10-recuris-rlmopt-next-levers.yaml'):
    ...
    ready=dict(schema='procedural-card-headroom-ready-v1',status='CPU_READY_MAIN_ACCEPTANCE_REQUIRED',source_sha256=sources,input_sha256=inputs,owner_argv=[str(s.NATIVE),str(s.ROOT/'ph_owner.py'),'run','--output',str(s.ATTEMPT)],verify_argv=[str(s.NATIVE),str(s.ROOT/'ph_owner.py'),'verify'],work_seconds=2550,owned_seconds=2670,outer_seconds=2700,startup_seconds=180,collection_seconds=2340,harvest_seconds=30,release_seconds=120,outer_margin_seconds=30,planned_full=48,planned_first_action=0,parent_clusters=8,paired_blocks=24,arms=['U','P'],policy_order=['sft24'],no_training=True,card_sha256=s.sha(s.ROOT/'CARD.txt'),card_tokens=proof['card_tokens'],all_original_task_files_unchanged=True,control_native_prefix_unchanged=True,explicit_algorithm_instruction_context_package=True,no_primitive_regression_estimand=True,main_launch_only=True,created_epoch=time.time())
