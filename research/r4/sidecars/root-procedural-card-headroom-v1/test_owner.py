...
def test_actual_owner_service_entry(tmp_path):
    assert Path(__file__).with_name('ph_owner.py').exists(), 'bounded48 owner not implemented'
    ...
    def popen(argv,**kwargs):
        spawned.append(argv)
        if len(spawned)==1:
            assert len(s.read(output/'PLANNED_EVALUATION.json')['full'])==48
    ...
    assert len(result['readout_inventory'])==48
...
def test_missing_result_and_late_physical_never_erase_cost(tmp_path):
    ...
    assert len(inventory)==48 and sum(r['available'] for r in inventory)==1
