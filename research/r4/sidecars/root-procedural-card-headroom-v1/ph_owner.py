"""Single fixed24 phase with explicit2700/2670/2550 clocks and complete48 inventory."""
import argparse
import functools
import os
...
def old_owner():return s.load('procedural-card_qualified_ledger',s.ss.CT/'ct_owner.py',s.ss.ct_ready['source_sha256'][str(s.ss.CT/'ct_owner.py')],{'ct_study':s.ss.ct})
...
def remaining(deadline,cap):
    ...
    if value<=0:raise TimeoutError('procedural-card inclusive stage deadline')
    return value
def collector_argv(stage,destination,deadline):return [str(s.NATIVE),str(s.ROOT/'ph_collect.py'),'--mode','free','--plan','FREE_PLAN.json','--start','0','--stop','48','--binding',str(stage/'BINDING.json'),'--endpoint',str(stage/'service/endpoint-original.json'),'--output',str(destination),'--deadline',str(float(deadline))]
def harvest(output,plan):
    ...
    return rows
def execute(output):
    started=time.time();work=started+2550;owned=started+2670
    if output.resolve()!=s.ATTEMPT.resolve() or output.exists():raise ValueError('exact unused procedural-card attempt only')
    ...
    s.write(output/'OWNER_RUN.json',dict(identity=ready['identity'],started_epoch=started,work_deadline_epoch=work,owned_deadline_epoch=owned,outer_seconds=2700,policy_order=['sft24'],gpu=gpu,no_training=True,model_action_tokens=2048,context_tokens=8192,selected=s.selected()))
    def expired(sig,frame):
        if sig in (signal.SIGTERM,signal.SIGINT):raise MainTermination('MAIN termination')
        raise TimeoutError('procedural-card stage/shared cap')
    ...
    try:
        ...
        end=time.time()+remaining(work-30,2340);signal.setitimer(signal.ITIMER_REAL,remaining(end,2340))
        suite.command(stage,'free48',collector_argv(stage,output/'sft24/free',end),remaining(end,2340),end);status['work_complete']=True
