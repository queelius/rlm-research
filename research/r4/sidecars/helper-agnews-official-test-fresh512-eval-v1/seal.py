"""CPU-qualify and seal four separately launchable official-test arms."""
...
def main():
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(study.ready_path(arm).exists() for arm in study.ARMS):
        raise ValueError("CPU-only unused seal required")
    started = time.monotonic()
    command = [str(study.NATIVE), "-m", "pytest", "-q", "-p", "no:cacheprovider", "test_fixture.py"]
    tests = subprocess.run(command, cwd=study.ROOT, capture_output=True, text=True, timeout=180)
    if tests.returncode:
    ...
    study.write_x(
        study.ROOT / "CPU_EVIDENCE.json",
        {
            ...
            "actual_four_endpoint_qualification": {arm: {key: value for key, value in receipt.items() if key != "binding"} for arm, receipt in qualified.items()},
            ...
            "python": sys.version,
            "max_prompt_plus_1024": max(len(row["body"]["token_ids"]) + 1024 for row in study.schedule()),
        },
    )
    closure = {}
    ...
    for arm in ("c32", "rl_step8", "sft_step8"):
        path = study.SOURCE / ("READY_" + arm.upper() + ".json")
        ready = study.read(path)
        closure.update(ready["closure_sha256"])
        closure[str(path)] = study.sha(path)
    seed2_ready = study.SEED2_EVAL / "READY_RL_SEED2_STEP8.json"
    seed2 = study.read(seed2_ready)
    closure.update(seed2["closure_sha256"])
    closure[str(seed2_ready)] = study.sha(seed2_ready)
    closure[str(eligibility.SEED2_QUALIFIED)] = study.sha(eligibility.SEED2_QUALIFIED)
    ...
    for arm in study.ARMS:
        ...
        print({"arm": arm, "ready_sha256": study.sha(study.ready_path(arm)), "identity": ready["identity"], "command": ready["command"]})
