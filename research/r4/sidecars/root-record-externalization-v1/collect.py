"""16 direct-source native interface roots, with physical-cost and NULL ledgers."""
import argparse
import asyncio
import json
import os
...
async def run(args):
    ...
    s.write(args.output / 'RUN.json', dict(identity=ready['identity'], endpoint=endpoint, started_epoch=started,
                                          deadline=args.deadline, root_deadline=root_deadline, planned=16, acquisitions=0))
    ...
    status = dict(planned=16, recorded=len(values), available=sum(r['available'] for r in values), correct=sum(r['reward'] == 1 for r in values),
                  physical_root_pipeline_attempts=sum(r.get('physical_attempts', 0) for r in values),
                  complete=len(values) == 16, all_native_finals_available=all(r['available'] for r in values), elapsed_seconds=time.time()-started)
