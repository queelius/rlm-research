"""Bounded single-process owner for the no-service AG SFT comparator."""
...
def execute(outer_seconds):
    if outer_seconds != study.CAP:
        raise ValueError("exact900 owner cap required")
    if study.ATTEMPT.exists():
        raise ValueError("preserve existing attempt")
    ...
    terminal = {
        "schema": "agnews-eightstep-answer-only-sft-owner-terminal-v1",
