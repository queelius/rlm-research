...
def label_spans(target, values):
    ...
    for value in values:
        ...
        # The diagnostic is categorical value content only. JSON quotes remain
        # structural even when the tokenizer fuses them with adjacent text.
        spans.append((start + 1, start + len(needle) - 1))
...
def plan():
    ...
    return {
        "schema": "agnews-eightstep-answer-only-sft-ready-v2",
        ...
        "output": str(ATTEMPT),
        "command": [str(TRAIN_PYTHON), str(ROOT / "owner_v2.py"), "run", "--outer-seconds", str(CAP)],
...
def endpoint():
    ...
    return {
        ...
        "ready_sha256": sha(ROOT / "READY_V2.json"),
