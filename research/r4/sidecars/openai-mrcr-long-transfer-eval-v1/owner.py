"""One bounded owned service per frozen long-transfer arm."""
...
def execute(stage: str, output: Path, outer_seconds: int) -> dict:
    spec = STAGES[stage]
    if output.resolve() != spec["output"].resolve() or output.exists():
    ...
    def stop(*_):
        raise TimeoutError("long-transfer evaluation owner signal")
    ...
    try:
        ...
        argv = [
            str(study.NATIVE), str(study.ROOT / "collect.py"),
            "--phase", "long", "--arm", spec["arm"],
            "--endpoint", str(endpoint), "--output", str(output / "science"),
            "--deadline", str(science_deadline),
        ]
        suite.command(
            service, "mrcr-long-" + stage, argv,
        ...
        complete = bool(
            not errors and released and result and result.get("complete") is True
