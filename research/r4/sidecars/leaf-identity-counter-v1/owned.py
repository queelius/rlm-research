"""One parent-launched identity_counter control; unchanged authenticated service lifecycle."""
import argparse
import hashlib
...
def load_suite():
    ...
    loader = importlib.util.spec_from_file_location("identity_counter_private_owned_suite", SUITE / "suite.py")
...
def service_binding(weights, weights_path, weights_sha):
    ...
    return {"schema": "identity_counter-control-single-old-adapter-binding-v1",
        ...
        "selection_semantics": "Existing old c32de validation-selected child only; all96 inputs frozen before outputs",
...
def execute(directory, suite, weights, started):
    ...
    s.write_once(directory / "WRAPPER_ATTEMPT.json", {"started_epoch": started,
        "overall_deadline_epoch": started + 1200, "work_deadline_epoch": work_deadline(started),
        "cleanup_reserve_seconds": 120, "collection_cap_seconds": 600, "collector_output": str(output),
    ...
    try:
        ...
        suite.command(directory, "identity_counter96-run", argv, collection_command_cap(started, time.time()), work_deadline(started))
        ...
        try:
            ...
            s.write_once(directory / "FINISH.json", {"elapsed_seconds": elapsed,
                "overall1200_exceeded": elapsed > 1200, "owned_release_completed": release_completed,
...
def main():
    ...
    def interrupted(sig, frame):
        raise KeyboardInterrupt(f"identity_counter owned signal {sig}; release authenticated owned service")
