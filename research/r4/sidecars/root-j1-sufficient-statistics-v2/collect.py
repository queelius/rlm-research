...
def implementation():
 ...
 try:spec=importlib.util.spec_from_file_location("ss_v2_collect_impl",path);m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
 finally:
...
def summarize(rows,plan,public,host):
 grouped={}
 for r in rows:grouped.setdefault(r["coordinate"]["context_id"],[]).append(r)
 expected={}
 for c in plan:expected.setdefault(c["context_id"],set()).add(c["id"])
 out=[]
 for ep in public:
  rs=grouped.get(ep["id"],[]);actual={r["coordinate"]["id"] for r in rs}
  merged={"status":"null","available":False,"valid":False,"answer":None} if actual!=expected[ep["id"]] else p.merge_chunks(rs,rs[0]["coordinate"]["users"])
...
async def run(endpoint,output,deadline,transport=None):
 m=implementation();v=await m.run(endpoint,output,deadline,transport);plan=s.read(s.ROOT/"inputs/PLAN.json");eps=summarize(v["rows"],plan,s.read(s.ROOT/"inputs/PUBLIC.json"),s.read(s.ROOT/"inputs/HOST_GOLD.json"));s.write(Path(output)/"EPISODES.json",eps);s.write(Path(output)/"SUMMARY.json",{"planned":8,"valid":sum(x["valid"] for x in eps),"null":sum(x["status"]=="null" for x in eps),"observed_invalid":sum(x["status"]=="observed_invalid" for x in eps),"strict":sum(x["strict"] is True for x in eps),"root_model_calls":0,"runtime_fallback":False});return v
def main():
