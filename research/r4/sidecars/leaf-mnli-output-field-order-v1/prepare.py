...
def main():
    ...
    for row in rows:
        ...
        for label in p.LABELS:
            good=[{k:({'label':label,'tag':t})[k] for k in p.field_order(row['arm'])} for t in p.requested_tags(context)]
            assert accepts(good)
        ...
        checks.append(dict(id=row['id'],arm=row['arm'],prompt_tokens=len(prompts[row['id']]),
                           correct_order_accepts_all_labels=True,opposite_order_rejected=True))
