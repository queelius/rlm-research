...
def collector_argv(stage,destination,deadline):return [str(s.NATIVE),str(s.ROOT/'ct_collect.py'),'--mode','free','--plan','FREE_PLAN.json','--start','0','--stop','48','--binding',str(stage/'BINDING.json'),'--endpoint',str(stage/'service/endpoint-original.json'),'--output',str(destination),'--deadline',str(float(deadline))]
...
def training_receipt():
    ...
    return dict(selected=selected,actual_artifact_sha256=pins,training_ready_sha256=s.TRAIN_READY_SHA,training_elapsed_seconds=owner['elapsed_seconds'],historical_training_outer_cap=4320,readout_outer_cap=6480,historical_training_not_new_cost=True,actual24_authenticated_before_service=True)
...
def ledger(output):
    ...
    return dict(full_native=dict(physical_requests_attempted=len(attempts),returned_native_completions=sum(returned(r) for r in attempts),failed_or_unconfirmed_completions=sum(not returned(r) for r in attempts),usage=usage(attempts)),records_sha256={str(p):s.sha(p) for p in free_paths},training_and_historical_acquisition_not_counted_as_new_eval=True,billing='unknown/not measured',attempts_are_not_all_confirmed_GPU_generations=True)
...
def execute(output):
    ...
    s.write(output/'OWNER_RUN.json',dict(identity=ready['identity'],started_epoch=started,work_deadline_epoch=work,owned_deadline_epoch=owned,outer_seconds=6480,historical_training_not_new_cost=True,policy_order=plan['policy_order'],same_root_role=True,model_action_tokens=2048,context_tokens=8192,gpu=gpu,no_training=True))
    ...
    try:
        for index,policy in enumerate(plan['policy_order']):
            ...
            try:
                ...
                collection=time.time()+remaining(end-90,2730);signal.setitimer(signal.ITIMER_REAL,remaining(collection,2730))
                suite.command(stage,'free48',collector_argv(stage,output/policy/'free',collection),remaining(collection,2730),collection)
        ...
        for row in plan['full']:
            directory=output/row['policy']/'free'/row['coordinate']['id'];path=directory/'RESULT.json'
            physical=list((directory/'physical').glob('*.json'));failure=directory/'FAILURE.json'
            value=dict(row,result_path=str(path),recorded=False,available=False,reward=None,operational_success=0,cause='not_started_no_artifacts',physical_records=len(physical))
            if path.exists():
                r=s.read(path);value.update(recorded=True,available=r['available'],reward=r['reward'],operational_success=int(r['available'] and r['reward']==1),result_sha256=s.sha(path),cause='native_final' if r['available'] else 'attempted_unfinished_native')
            elif failure.exists() or physical:
                value.update(cause='attempted_exception_no_result' if failure.exists() else 'attempted_interrupted_no_result',failure_sha256=s.sha(failure) if failure.exists() else None)
            elif directory.exists():value['cause']='started_no_physical_request_record'
            full.append(value)
        s.write(output/'COST_LEDGER.json',ledger(output))
        result=dict(identity=ready['identity'],complete=not errors and len(stages)==2,error=errors or None,stages=stages,readout_inventory=full,planned_full=96,planned_first_action=0,released=active is None,active_unreleased_service=str(active) if active else None,elapsed_seconds=time.time()-started,native_availability_not_implied_by_orchestrator_completion=True,no_partial24_substitution=True,no_training=True)
