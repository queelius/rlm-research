"""MAIN-only1650work/1770owned/1800outer owner; no retry or root inference."""
import argparse
import os
...
def execute(output):
    ...
    s.write(output/'PLANNED.json',plan);s.write(output/'OWNER_RUN.json',dict(identity=ready['identity'],started_epoch=started,work_deadline=work,owned_deadline=owned,outer_seconds=1800,startup_cap=180,collector_harvest_reserve=30,release_cap=90,finalize_reserve=30,outer_margin=30,scientific_role='leaf_base_vs_c32',credential_present=True,gpu=gpu))
    ...
    try:
        ...
        suite.command(stage,'leaf152',collector_argv(stage,output/'rollout',work),max(.001,work-time.time()),work)
