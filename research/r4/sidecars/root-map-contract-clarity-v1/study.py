"""Pinned existing native runtime, new map-contract clarity task only."""
import functools
import hashlib
...
ANCHOR = ROOT.parent / 'root-example-map-visibility-v1'
MAP_SOURCE = ROOT.parent / 'root-supplied-map-reducer-v1'
...
def source():
    return load('clarity_complete_source', SOURCE / 'study.py',
...
def binding():
    prior = load('clarity_rl4_binding_source', RL4 / 'study.py',
...
def task(context, prompt, gold, row, map_bytes=None):
    original = stack().native.task(context, prompt, gold, row['id'])
    if map_bytes is not None and not isinstance(map_bytes, bytes):
        raise TypeError('supplied map must be the frozen exact byte payload')
    ...
    class SuppliedMapTask(type(original)):
        async def setup(self, trace, runtime):
            ...
            if map_bytes is not None:
                await runtime.write('labels.json', map_bytes)
