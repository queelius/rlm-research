"""Additive MAIN-owned launcher; original clarity READY and source stay frozen."""
import argparse
import functools
import os
import signal
...
def credential_preflight():
    credential = s.load('clarity_credential_preflight', RUNTIME / 'credential_preflight.py',
...
def dependencies():
    ...
    with s.source().aliases({'study': original}):
        launcher = s.load('clarity_qualified_owned_dependencies', original.ROOT / 'launch.py',
                          '62d7d0049b5e3303f6e68247038290c7cde0d74e3394b4856fea0e3e138d6b46')
    suite = launcher.dependencies()
    previous = list(sys.path)
    sys.path.insert(0, str(RUNTIME))
    try:
        adapter = s.load('clarity_actual_service_lifecycle', RUNTIME / 'lifecycle_adapter.py',
...
def execute(output):
    ...
    s.write(output / 'OWNER_RUN.json', dict(owner_identity=ready['identity'], scientific_ready_sha256=ready['scientific_ready_sha256'],
            ...
            outer_seconds=1800, cleanup_seconds=120, gpu=gpu, **credential))
    def expired(_sig, _frame):
        raise TimeoutError('owned1770-second inclusive deadline or MAIN termination')
    ...
    try:
        ...
        argv = [str(s.NATIVE), str(s.ROOT / 'collect.py'), '--binding', str(stage / 'BINDING.json'),
                ...
                '--deadline', str(work)]
        suite.command(stage, 'clarity-collect', argv, 1650, work)
    ...
    result = dict(complete=error is None and released, error=error, release_error=release_error,
                  released=released, collector_terminal=terminal, planned=96, acquisitions=0, reused_acquisitions=4,
                  work_deadline_epoch=work, owned_deadline_epoch=owned, elapsed_seconds=time.time() - started,
                  collector_output=str(output / 'rollout'), outer_seconds=1800,
...
def seal():
    ...
    value = dict(status='CPU_READY_OWNED_SUCCESSOR_FOR_MAIN_ACCEPTANCE', source_sha256=sources,
                 ...
                 parent_outer_seconds=1800, owned_inclusive_seconds=1770, work_seconds=1650, cleanup_seconds=120,
                 service_readiness_cap_seconds=180, shared_clock='owner start through service startup plus96 endpoints;4 completed acquisitions reused, not newly paid',
                 argv=[str(s.NATIVE), str(s.ROOT / 'owner.py'), 'run', '--output', str(ATTEMPT)],
                 verify_argv=[str(s.NATIVE), str(s.ROOT / 'owner.py'), 'verify'],
                 output_nesting='owner at outputs/attempt-001; service at owned-service; clarity collector at rollout',
