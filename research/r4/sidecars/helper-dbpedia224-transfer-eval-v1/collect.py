"""Native token decoder with exact DBpedia labels and raw envelope persistence."""
...
def validate_prediction(value, ids):
    if not isinstance(value, dict) or list(value) != ids or set(value) != set(ids):
        raise ValueError("prediction does not contain exact requested IDs in order")
    if any(label not in study.LABELS for label in value.values()):
        raise ValueError("prediction contains a non-canonical DBpedia label")
    return value
...
def decode(row, response, tokenizer, started, ended):
    ...
    try:
        ...
        result["prediction"] = validate_prediction(parsed, row["ids"])
        result["status"] = "returned_valid"
