        ...
        try:
            ...
            if mode=='capture':
                ...
                s.write(target/'RESULT.json',dict(coordinate=row,mode=mode,**score,gold=answer,
                    ...
                    final_branch_token_identity_verified=True,physical_cost=p.physical_cost(paid),
...
async def run(args):
    ...
    if args.mode=='capture':
        ...
        if args.plan=='TRAIN_PLAN.json' and args.stop==32:
            s.corpus(32);files={str(p):s.sha(p) for p in args.output.rglob('*.json')}
            s.write(args.output/'CORPUS_READY.json',dict(identity=s.verify()['identity'],examples=32,files_sha256=files,host_gold_used=False))
    else:
        from verifiers.v1.envs.single_agent import SingleAgentEnv,SingleAgentEnvConfig
        ...
        async def worker():
            while not queue.empty() and time.time()<args.deadline:
                ...
                try:await episode(row,public[row['context_id']],binding,descriptor,args.output/row['id'],args.deadline,args.mode,
                    s.ATTEMPT/'controlled-source'/row['id'] if args.mode=='controlled' else None,installed=(env,interface))
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--mode',choices=('capture','free','controlled'),required=True);ap.add_argument('--plan',choices=('TRAIN_PLAN.json','FREE_PLAN.json','CONTROLLED_PLAN.json'),required=True)
