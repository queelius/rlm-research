"""Four-worker checkpointed collector for fresh positional-anchor192."""
...
async def run(endpoint_path, output, deadline, transport=None):
    ready = s.verify(); contexts = p.contexts(); plan = s.read(s.ROOT / "PLAN.json"); bodies = s.read(s.ROOT / "REQUESTS.json"); wires = s.read(s.ROOT / "ORDERED_REQUESTS.json"); prompts = s.read(s.ROOT / "PROMPT_IDS.json"); native = s.read(s.ROOT / "CPU_NATIVE.json"); tokenizer = s.tokenizer(); endpoint = s.read(endpoint_path)
    ...
    async with httpx.AsyncClient(headers=headers, trust_env=False, transport=transport, timeout=90, event_hooks={"request": [dispatched]}) as client:
        async def worker():
            while not queue.empty() and time.time() < call_deadline:
                row = queue.get_nowait(); key = row["id"]; context = contexts[row["context_index"]]
                ...
                try:
                    ...
                    request = client.build_request("POST", base + "/v1/chat/completions", content=wire); request.extensions["science_checkpoint"] = {"directory": directory, "prompt": prompts[key], "result": result}
                    response = await asyncio.wait_for(client.send(request), timeout=min(90, max(0.001, call_deadline - time.time())))
                    result["status_code"] = response.status_code; s.write(directory / "RESPONSE.json", {"status_code": response.status_code, "body": response.text, "received_epoch": time.time()}); raw = response.json(); result["raw"] = raw
                    usage = raw.get("usage") if isinstance(raw, dict) else None
                    if isinstance(usage, dict) and all(type(usage.get(name)) is int and usage[name] >= 0 for name in ("prompt_tokens", "completion_tokens")): result["usage_observed"] = {name: usage[name] for name in ("prompt_tokens", "completion_tokens")}
                    response.raise_for_status(); message, finish, _ = scoring.verified_response(raw, prompts[key], tokenizer)
        ...
        try: await asyncio.wait_for(asyncio.gather(*tasks), timeout=max(0.001, call_deadline - time.time()))
        except BaseException as caught:
    ...
    status = {"planned": 192, "recorded": len(rows), "available": sum(row["score"]["available"] for row in rows), "physical_attempts": sum(row["physical_attempt"] for row in rows), "inventory_complete": len(rows) == 192, "all_native_finals_available": all(row["score"]["available"] for row in rows), "orchestrator_error": failure, "elapsed_seconds": time.time() - started}
    s.write(output / "STATUS.json", status); return status


if __name__ == "__main__":
    parser = argparse.ArgumentParser(); parser.add_argument("command", choices=("run",)); parser.add_argument("--endpoint", required=True, type=Path); parser.add_argument("--output", required=True, type=Path); parser.add_argument("--deadline", required=True, type=float); args = parser.parse_args()
