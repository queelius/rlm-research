"""Shared source-bound accumulation; coverage-first changes only displayed counts."""
import copy
import hashlib
import json
...
class Ledger:
    def __init__(self,root_id,config,request_for,strict_map,audit_path=None):
        ...
        if self.arm not in ('map','always_counts','coverage_first'):raise ValueError('unknown ledger arm')
    ...
    def observe(self,invocation,original_truncated,raw_original=None):
        ...
        displayed={k:v for k,v in summary.items() if k!='counts'} if self.arm=='coverage_first' and summary['partial'] else summary
        if self.arm in ('always_counts','coverage_first') and any(e['event']=='delivered' and e['eligible'] for e in self.events):
            suffix='\n\n[Harness accumulation ledger: counts of returned child labels, not verified truth; unresolved/partial coverage is explicit.]\n'+json.dumps(displayed,separators=(',',':'))
        ...
        self.emit({'event':'root_observation','raw_original':raw_original,'original_truncated':original_truncated,
                   'appended_summary':bool(suffix),'summary':summary,'displayed_summary':displayed,'actual_content':content,
