"""Single fixed24 phase with explicit1800/1770/1650 clocks and complete32 inventory."""
import argparse
import functools
import os
...
def old_owner():return s.load('accumulation_qualified_ledger',s.ss.CT/'ct_owner.py',s.ss.ct_ready['source_sha256'][str(s.ss.CT/'ct_owner.py')],{'ct_study':s.ss.ct})
...
def remaining(deadline,cap):
    ...
    if value<=0:raise TimeoutError('accumulation inclusive stage deadline')
    return value
def collector_argv(stage,destination,deadline):return [str(s.NATIVE),str(s.ROOT/'ae_collect.py'),'--mode','free','--plan','FREE_PLAN.json','--start','0','--stop','32','--binding',str(stage/'BINDING.json'),'--endpoint',str(stage/'service/endpoint-original.json'),'--output',str(destination),'--deadline',str(float(deadline))]
...
def execute(output):
    ...
    if output.resolve()!=s.ATTEMPT.resolve() or output.exists():raise ValueError('exact unused accumulation attempt only')
    ...
    s.write(output/'OWNER_RUN.json',dict(identity=ready['identity'],started_epoch=started,work_deadline_epoch=work,owned_deadline_epoch=owned,outer_seconds=1800,policy_order=['sft24'],gpu=gpu,no_training=True,model_action_tokens=2048,context_tokens=8192,selected=s.selected()))
    def expired(sig,frame):
        if sig in (signal.SIGTERM,signal.SIGINT):raise MainTermination('MAIN termination')
        raise TimeoutError('accumulation stage/shared cap')
    ...
    try:
        ...
        end=time.time()+remaining(work-30,1440);signal.setitimer(signal.ITIMER_REAL,remaining(end,1440))
        suite.command(stage,'free32',collector_argv(stage,output/'sft24/free',end),remaining(end,1440),end);status['work_complete']=True
