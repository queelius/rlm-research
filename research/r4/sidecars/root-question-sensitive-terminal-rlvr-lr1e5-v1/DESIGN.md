---
date: 2026-09-10
status: cpu_implementation_pending_main_review
...
2. Step greater than zero accepts only the last committed policy path below this new attempt;
   recorded noop windows may lie between that checkpoint and the current fixed cursor. Authenticate
   adapter, state, optimizer, RNG, generation, parameter order, campaign ID, and actual cursor
...
5. The next trainer process authenticates and restores the committed adapter/Adam/RNG hashes,
   parameter order, cursor, and full parameter-group dictionary; explicit LR and weight-decay
   assertions run both before and after restore. Independent post-terminal audit will deserialize
   every materialized checkpoint and recount moment/RNG inventory. A mismatch never falls back to
...
Every actual update commits an atomic window-local checkpoint. The owner attempts the eight fixed
windows in order but may stop early when fewer than 2,250 seconds remain or on a recorded training
error. It still reads out the last actual commit, including QS6 if there are zero updates. Reports
must distinguish `completed_windows`, `actual_optimizer_step`, and readout completion; a clean
readout after an early stop is never called a full-eight-window intervention. There is no
best-checkpoint, validation, or protected-outcome selection.
...
The inherited producer cost ledger has historically undercounted this native role-audit layout.
Treat it as an incomplete convenience artifact. The independent outcome audit must enumerate raw
`role-audit/*-request.json` and `*-result.json` records and is authoritative for physical calls,
responses, usage-known/unknown counts, and hashes.
...
No READY seal, GPU service, or launch is authorized by this CPU preparation.

## Numerical implementation disclosure

The producer reuses the qualified sparse-head likelihood implementation: it requests logits only
at credited action positions while preserving the same terminal reward, TIS/PPO expression, masks,
and episode/turn weighting. The original high-LR campaign's first update used a dense-logit forward.
The LR comparison is therefore objective-matched but **not bitwise** a one-factor numerical
replication. Report this implementation difference alongside the LR intervention.

The outcomes audit must report agent-reviewed `faithful+strict` task execution from actual programs,
tool observations, and final state alongside raw answer accuracy. A correct final caused by a
gold-zero coincidence is not promoted as faithful execution.
