"""Additive LR1e-4 qualifier; only expected optimizer/state LR differs from accepted source."""
...
def validate_state_binding(state,binding):
    required={'status':'UPDATED','optimizer_steps':1,'fresh_AdamW':True,'optimizer_state_steps':[1],
        ...
        'replay_passed':True,'replay_actions':12,'learning_rate':1e-4,'token_TIS_cap':2.,
...
def verify_checkpoint():
    ...
    assert pg['lr']==1e-4 and pg['weight_decay']==0 and tuple(pg['betas'])==(.9,.999) and pg['eps']==1e-8
    ...
    return dict(eligible=True,checkpoint=str(CHECKPOINT),step_commit_sha256=study.sha(RECEIPT),
        ...
        replay_actions=12,zero_advantage_skipped=20,optimizer_steps=1,learning_rate=1e-4,adapter_delta_l2=delta,
