...
import httpx
import collect,mn_protocol as p,mn_study as s
...
class CollectorTests(unittest.TestCase):
    ...
    def test_actual_32_wires_preserve_schema_native_branches_and_nulls(self):
        ...
        def response(request):
            ...
            if key==plan[3]['id']:raw['choices'][0]['finish_reason']='length'
        ...
        with tempfile.TemporaryDirectory(prefix='exact-tag-transport-') as tmp:
            ...
            self.assertTrue(actual[3]['length_capped']);self.assertTrue(actual[3]['score']['available'])
            self.assertEqual(len(list((output/'calls').glob('*/RESPONSE.json'))),32)
            with patch.object(s,'verify',lambda:{'identity':'cpu'}),patch.object(s,'tokenizer',lambda:self.tok),patch.object(s.service,'validate_descriptor',lambda *a:None),patch.dict(os.environ,{'EXACT_TAG_FIXTURE':'fixture'}):
                with self.assertRaises(FileExistsError):asyncio.run(collect.run(endpoint,output,time.time()+120,transport=httpx.MockTransport(response)))
