...
def credential():
    return s.base.load("position_anchor_credential", s.base.RUNTIME / "credential_preflight.py", "2ff11844d7237f99d1d6080b2e599a1bacf34ad693e13acd98a41cddcaaa8110").require_provider_credential()
...
def suite():
    value = s.base.load("position_anchor_suite", s.SIDE / "leaf-post-sft-suite-v1/suite.py", "6fa84af486efbf5bad17352553d36cd590fb6f7e386c2273df27dd8deb7c8fd1")
    value.verify()
...
def execute(output):
    ...
    runner = suite()
    started = time.time()
    work, owned = started + CLOCK["work"], started + CLOCK["owned"]
    output.mkdir(parents=True)
    stage = output / "owned-service"
    stage.mkdir()
    s.write(output / "PLANNED_NULL_ENDPOINTS.json", [p.null_row(row, "before service startup") for row in s.read(s.ROOT / "inputs/PLAN.json")])
    ...
    def expired(*_):
        raise TimeoutError("owned1770 deadline or MAIN termination")
    ...
    error = release_error = terminal = None
    released = False
    try:
        runner.start_service(stage, binding(), min(work, time.time() + CLOCK["startup"]))
        runner.command(stage, "mnli-position-anchor96-collect", collector_argv(stage, output, work), max(0.001, work - time.time()), work)
        terminal = s.read(output / "rollout/STATUS.json")
        if terminal["planned"] != 96 or terminal["recorded"] != 96:
            raise ValueError("collector must retain 96 slots")
    except BaseException as caught:
        error = {"type": type(caught).__name__, "message": str(caught)}
    finally:
        signal.setitimer(signal.ITIMER_REAL, max(0.001, min(owned - CLOCK["finalize"], time.time() + CLOCK["release"]) - time.time()))
        try:
            runner.release_service(stage)
            released = True
        except BaseException as caught:
            release_error = {"type": type(caught).__name__, "message": str(caught)}
        signal.setitimer(signal.ITIMER_REAL, max(0.001, owned - time.time()))
    result = {"complete": error is None and released, "error": error, "release_error": release_error, "released": released, "active_unreleased_service": None if released else str(stage), "collector_status": terminal, "planned": 96, "elapsed_seconds": time.time() - started, "no_retry": True}
