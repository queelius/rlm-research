# AG News B4 native/HF one-step fresh-seed replica

Does the prior one-win/zero-loss result persist when the exact same frozen128
training records and protocol are rerun from fresh c32/Adam with prespecified new
native and HF seeds? This changes no article, prompt, reward, gate, or optimizer setting.
...
The parent experiment bundled domain/data, request granularity, reward and dose.
This replica varies only sampling/optimizer seeds and coordinate identity. Persistence
would motivate broader prospective evaluation, not a confirmed improvement.
