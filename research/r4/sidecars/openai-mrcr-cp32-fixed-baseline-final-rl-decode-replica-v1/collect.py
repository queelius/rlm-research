"""Shallow qualified native collector with fresh paired seeds, unchanged hooks."""
import argparse
import asyncio
...
try:source=study.load('decode_replica_original_science',study.SOURCE_EVAL/'collect.py')
finally:
...
async def run(phase,arm,endpoint,output,deadline):
    if phase!=arm or arm not in study.CAPS:raise ValueError('arm/phase binding differs')
    with hooks.installed() as contract:
        code=await source.run(phase,arm,endpoint,output,deadline)
        study.write_x(output/'TERMINAL_STRIP_CONTRACT.json',contract);return code
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--phase',choices=tuple(study.CAPS),required=True);p.add_argument('--arm',choices=tuple(study.CAPS),required=True)
