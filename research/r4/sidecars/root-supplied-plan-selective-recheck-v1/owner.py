"""Bounded MAIN-only owner for 24 selective recheck calls."""
import argparse
import os
...
def execute(output):
    ...
    suite=s.dependencies();plan=s.read(s.ROOT/"inputs/PLAN.json");output.mkdir(parents=True);stage=output/"owned-service";stage.mkdir();s.write(output/"PLANNED.json",plan)
    s.write(output/"OWNER_RUN.json",{"identity":ready["identity"],"started_epoch":started,"work_deadline":work,"owned_deadline":owned,"outer_seconds":1800,"planned_child_calls":24,"root_model_calls":0,"runtime_fallback":False,"credential_present":True,"gpu":gpu})
    def expired(sig,frame):
        if sig in (signal.SIGINT,signal.SIGTERM):raise MainTermination("MAIN cancellation")
        raise TimeoutError("bounded selective recheck cap")
    handlers={sig:signal.signal(sig,expired) for sig in (signal.SIGINT,signal.SIGTERM,signal.SIGALRM)};errors=[];active=True
    try:
        startup=min(work-30,time.time()+180);signal.setitimer(signal.ITIMER_REAL,max(.001,startup-time.time()));suite.start_service(stage,s.binding(),startup);signal.setitimer(signal.ITIMER_REAL,max(.001,work-time.time()));suite.command(stage,"selective-recheck24",collector_argv(stage,output/"rollout",work),max(.001,work-time.time()),work)
    except BaseException as error:errors.append({"stage":"work","type":type(error).__name__,"message":str(error)})
    finally:
...
def main():
    ...
    if args.command=="verify":print(s.verify()["identity"])
        ...
        result=execute(args.output);print({"complete":result["complete"],"released":result["released"]});raise SystemExit(0 if result["complete"] else 1)
