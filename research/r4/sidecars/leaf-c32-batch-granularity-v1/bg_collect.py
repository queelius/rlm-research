...
def summarize(rows,gold,public):
    cells={}
    for row in rows:
        c=row['coordinate'];key=(c['context_id'],c['repeat'],c['arm']);cells.setdefault(key,[]).append(row)
    result=[]
    for (context,repeat,arm),values in cells.items():
        ...
        result.append(dict(context_id=context,repeat=repeat,arm=arm,planned_labels=n,strict_correct=correct,strict_bounds=[correct,correct+missing],null_labels=missing,complete_maps=sum(r['score']['complete_map'] for r in values),planned_maps=len(values),canonical_id_matches=sum(r['score']['canonical_id_matches'] or 0 for r in values),host_target_scalar=scalar,host_target_gold=truth,host_target_error=scalar-truth if scalar is not None else None,host_target_is_not_root_execution=True))
    return result
