...
def _kind_ledger(output, kind):
    ...
    if kind == "free":
        ...
        returned_choices = sum(bool((row.get("response") or {}).get("choices")) for row in physical)
        ...
        returned_choices = sum(bool(row.get("choices")) for row in raw_responses)
        physical_attempts = sum(bool(row.get("physical_request_attempt")) for row in results)
    ...
    return {
        ...
        "record_sha256": {str(path): s.sha(path) for path in physical_paths + results_paths + response_paths},
    }
...
def verify_v2():
    ready = s.read(s.ROOT / "READY_v3.json")
