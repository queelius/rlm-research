...
def _kind_ledger(output, kind):
    episode_paths = sorted({path.parent for path in output.glob(f"*/{kind}/*/RESULT.json")})
    episode_paths += sorted({path.parent for path in output.glob(f"*/{kind}/*/FAILURE.json")
    ...
    return {
        ...
        "physical_requests": sum(bool(row.get("physical_request_attempt")) for row in physical)
                             if kind == "free" else sum(bool(row.get("physical_request_attempt")) for row in results),
        "returned_completions": sum(bool((row.get("response") or {}).get("choices")) for row in physical)
                                if kind == "free" else sum(bool(row.get("response_returned")) for row in results),
        "authenticated_completions": sum(bool(row.get("available")) for row in results),
        "usage": _usage(physical if kind == "free" else results),
        "record_sha256": {str(path): s.sha(path) for path in physical_paths + results_paths},
    }
