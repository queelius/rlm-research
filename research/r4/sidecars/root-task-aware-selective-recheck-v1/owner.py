"""Bounded MAIN-only owner for 48 task-aware recheck calls."""
...
def execute(output):
    ...
    s.write(output/"OWNER_RUN.json",{"identity":ready["identity"],"started_epoch":started,"work_deadline":work,"owned_deadline":owned,"outer_seconds":1200,"planned_child_calls":48,"root_model_calls":0,"runtime_fallback":False,"gpu":gpu,"credential_present":True})
    def expired(sig,frame):
        if sig in (signal.SIGINT,signal.SIGTERM):raise MainTermination("MAIN cancellation")
        raise TimeoutError("bounded task-aware cap")
    handlers={sig:signal.signal(sig,expired) for sig in (signal.SIGINT,signal.SIGTERM,signal.SIGALRM)};errors=[];active=True
    try:
        startup=min(work-30,time.time()+180);signal.setitimer(signal.ITIMER_REAL,max(.001,startup-time.time()));suite.start_service(stage,s.binding(),startup);signal.setitimer(signal.ITIMER_REAL,max(.001,work-time.time()));suite.command(stage,"task-aware-recheck48",collector_argv(stage,output/"rollout",work),max(.001,work-time.time()),work)
...
def main():
    ...
    if a.command=="verify":print(s.verify()["identity"])
        ...
        result=execute(a.output);print({"complete":result["complete"],"released":result["released"]});raise SystemExit(0 if result["complete"] else 1)
