...
def verify_checkpoint():
    ...
    assert replay['passed'] and replay['optimizer_steps']==0 and len(replay['replay'])==32
    rows=study.train.validate_inputs(study.read(study.train.INPUTS))
    token_math=study.load('fixedRL_eval_token_math',study.train.PRIOR/'math_core.py')
    for i,(row,actual,check) in enumerate(zip(rows,replay['episodes'],replay['replay'],strict=True)):
        assert actual['episode_id']==check['episode_id']==row['episode_id'] and check['passed']
    ...
    return dict(eligible=True,checkpoint=str(CHECKPOINT),step_commit_sha256=study.sha(RECEIPT),
        ...
        replay_actions=32,optimizer_steps=1,adapter_delta_l2=delta,
