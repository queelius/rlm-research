"""Pinned exact collector adapted only from 32 to the frozen 48-row plan."""
import sys,types
...
module=types.ModuleType('alien_exact_collector');module.__file__=str(path);sys.modules[module.__name__]=module
source=source.replace(old,'status=dict(planned=48,recorded=len(rows)').replace(inventory,'inventory_complete=len(rows)==48')
with s.aliases({'study':s,'protocol':p,'scoring':scoring}):exec(compile(source,str(path)+'::plan48','exec'),module.__dict__)
run,summarize=module.run,module.summarize
if __name__=='__main__':
    import argparse,asyncio,owner
