...
class OwnerTests(unittest.TestCase):
    def test_actual_owner_start_failure_keeps80_slots_and_cleanup_budget(self):
        import os
        import tempfile
        ...
        with tempfile.TemporaryDirectory(prefix='interface-owner-failure-') as directory:
            ...
            self.assertEqual(run['work_deadline']-run['started_epoch'],2490)
            self.assertEqual(run['owned_deadline']-run['started_epoch'],2610)
            self.assertEqual(run['outer_seconds'],2700)
            self.assertEqual(len(original_read(output/'PLANNED_NULL_ENDPOINTS.json')),80)
    ...
    def test_full_service_entry_pins_qualified_module_namespace_before_launch(self):
        ...
        with tempfile.TemporaryDirectory(prefix='join-service-test-') as directory:
            root = Path(directory); binding = owner.binding(); output=root/'attempt-001'
            from types import SimpleNamespace
            def start(stage, actual_binding, deadline):
                self.assertEqual(actual_binding,binding)
                study.write(stage/'BINDING.json',actual_binding)
                with patch.object(sys,'argv',[str(study.ROOT/'service_wrapper.py'),'--binding',str(stage/'BINDING.json'),'--run-dir',str(stage/'service')]):
                    service_wrapper.main()
            def command(stage, name, argv, timeout, deadline):
                parsed=owner.validate_argv(argv);observed['collector_argv']=argv
                study.write(parsed['output']/'STATUS.json',dict(planned=80,recorded=80))
            released=[]
            runner=SimpleNamespace(start_service=start,command=command,release_service=lambda stage:released.append(stage))
            with patch.object(util, 'spec_from_file_location', spec), patch.object(subprocess, 'Popen', spawn), \
                 ...
                 patch.object(study,'ATTEMPT',output),patch.object(study,'verify',lambda:{'identity':'cpu'}), \
                 patch.object(owner,'credential',lambda:{}),patch.object(owner,'suite',lambda:runner):
                # Only not-yet-created READY hashing is substituted; real service sources remain authenticated.
                original_sha=study.sha
                with patch.object(study,'sha',lambda path:'cpu-ready' if Path(path)==study.READY_PATH else original_sha(path)):
                    result=owner.execute(output)
            self.assertTrue(result['complete']);self.assertEqual(len(released),1)
            service=output/'owned-service/service'
            self.assertEqual(observed['alias'], study.MODEL['alias'])
            self.assertEqual(observed['command'], ['/fixture/prime/bin/inference', '@', str(service/'inference.json')])
            self.assertEqual(study.read(service/'BINDING.json'), binding)
            self.assertIsNone(study.read(service/'endpoint-original.json')['adapter'])
            configuration=study.read(service/'inference.json')['vllm']
            self.assertFalse(configuration['enable_lora']);self.assertFalse(configuration['enable_prefix_caching'])
            self.assertEqual(configuration['max_model_len'],8192);self.assertEqual(configuration['max_num_seqs'],4)
            self.assertEqual(configuration['dtype'],'bfloat16')
            self.assertEqual(observed['collector_argv'][1],str(study.ROOT/'collect.py'))
    ...
    def test_actual_collector_cli_is_bound_to_new_attempt(self):
        ...
        with self.assertRaises(ValueError):
            owner.validate_argv([x.replace('leaf-mnli-correspondence-v1', 'root-partition-final-interface-v1') for x in argv])
