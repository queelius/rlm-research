"""MAIN-only released-base lifecycle; exact namespace and inclusive 2700s envelope."""
import argparse
import os
...
def execute(output):
    ...
    runner = suite(); started = time.time(); work = started + 2490; owned = started + 2610
    ...
    s.write(output / 'OWNER_RUN.json', dict(identity=ready['identity'], ready_sha256=s.sha(s.READY_PATH),
        started_epoch=started, work_deadline=work, owned_deadline=owned, outer_seconds=2700, gpu=gpu, **private))
    def expired(*_): raise TimeoutError('owned2610s deadline or MAIN termination')
    ...
    try:
        ...
        runner.command(stage, 'mnli-correspondence-collect', argv, max(.001, work-time.time()), work)
        terminal = s.read(output / 'rollout/STATUS.json')
        if terminal['planned'] != 80 or terminal['recorded'] != 80:
            raise ValueError('collector must retain80 planned slots')
    ...
    result = dict(complete=error is None and released, error=error, release_error=release_error,
                  released=released, collector_status=terminal, planned=80,
